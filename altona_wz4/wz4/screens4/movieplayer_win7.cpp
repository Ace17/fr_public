...
class sFakeCOMObject : public Interface
{
...
public:
  ...
  void Push(const sU8* data, sInt nBytes, sS64 timeStampSmp)
  {
    ...
    if(clen1)
      sCopyMem(cptr1, data, clen1);

    if(clen2)
      sCopyMem(cptr2, data + clen1, clen2);
...
public:
  ...
  struct Callbacks : public sFakeCOMObject<IMFSourceReaderCallback>
  {
    ...
    HRESULT STDMETHODCALLTYPE OnReadSample(HRESULT hrStatus, DWORD dwStreamIndex, DWORD dwStreamFlags, LONGLONG llTimestamp, IMFSample* pSample)
    {
  ...
  sBool InitMF()
  {
    ...
    static const sChar* fmtstr[] =
    {
      L"YV12", L"RGB32", L"IYUV", L"YUY2", L"UYVY",
    };
...
sMoviePlayer* sCreateMoviePlayer(const sChar* filename, sInt flags, sTextureBase* alphatexture)
{
  ...
  if(mp && !mp->Init(filename, flags, alphatexture))
    sDelete(mp);
