...
void RNFR063_Sprites::Init()
{
  ...
  {
  default: Mtrl->BlendColor = sMB_ADD;
    break;
  case 0x0001: Mtrl->BlendColor = sMB_PMALPHA;
    break;
  case 0x0002: Mtrl->BlendColor = sMB_MUL;
    break;
  case 0x0003: Mtrl->BlendColor = sMB_MUL2;
    break;
  case 0x0004: Mtrl->BlendColor = sMB_ADDSMOOTH;
    break;
  case 0x0005: Mtrl->BlendColor = sMB_ALPHA;
    break;
  }
  switch(Para.Mode & 0x0030)
  {
  default: Mtrl->Flags |= sMTRL_ZOFF;
    break;
  case 0x0010: Mtrl->Flags |= sMTRL_ZREAD;
    break;
  case 0x0020: Mtrl->Flags |= sMTRL_ZWRITE;
    break;
  case 0x0030: Mtrl->Flags |= sMTRL_ZON;
    break;
  }
  switch(Para.Mode & 0x30000)
  {
  default: Mtrl->BlendAlpha = sMBS_1 | sMBO_ADD | sMBD_0;
    break;
  case 0x10000: Mtrl->BlendAlpha = sMBS_0 | sMBO_ADD | sMBD_1;
    break;
  case 0x20000: Mtrl->BlendAlpha = sMBS_0 | sMBO_ADD | sMBD_0;
    break;
  case 0x30000: Mtrl->BlendAlpha = sMBS_1 | sMBO_ADD | sMBD_SAI;
    break;
  }
...
void RNFR063_Sprites::Render(Wz4RenderContext* ctx)
{
  ...
  switch(Para.Mode & 0x0030)
  {
  case 0x0020: case 0x0030: zwrite = sTRUE;
    break;
  default: zwrite = sFALSE;
    break;
  }
  ...
  if(ctx->IsCommonRendermode(zwrite))
  {
    ...
    switch(Para.Mode & 0x000f)
    {
    default: fade0col = 0x00000000;
      break;
    case 0x0001: fade0col = 0x00000000;
      break;
    case 0x0002: fade0col = 0x00ffffff;
      break;
    case 0x0003: fade0col = 0x00808080;
      break;
    case 0x0004: fade0col = 0x00000000;
      break;
    case 0x0005: fade0col = 0x00ffffff;
      break;
    }
...
void RNFR063ClothGridSimRender::Render(Wz4RenderContext* ctx)
{
     ...
     (ctx->RenderMode & sRF_TARGET_MASK) == sRF_TARGET_DIST)
  {
    ...
    if(Mtrl->SkipPhase(ctx->RenderMode, Para.LightEnv))
      return;
...
// Create Fix Mesh
  ...
  {
    FixVertices[i].nx = 0.0f;
    FixVertices[i].ny = 0.0f;
    FixVertices[i].nz = 0.0f;
  }
  ...
  for(i = 0; i < nas; i++)
  {
    ...
    if(i != (nas - 1))
      dir = p2 - p1;  // special case, keep previous dir
    ...
    for(int j = 0; j < Para.Slices; j++)
    {
      ...
      FixVertices[j].nx += p2.x;
      FixVertices[j].ny += p2.y;
      FixVertices[j].nz += p2.z;
    }
...
// sF32 totallen=len;
  ...
  for(i = 0; i < nas * Para.Slices; i++)
  {
    ...
    t.Unit();
    FixVertices[i].nx = t.x;
    FixVertices[i].ny = t.y;
    FixVertices[i].nz = t.z;
  }
...
void RNFR063AermelKanal::Simulate(Wz4RenderContext* ctx)
{
  ...
  // Init
  if(PathSymbol && PathSymbol->Value && PathSymbol->Value->Spline)
    PathSpline = PathSymbol->Value->Spline;

  if(MovementSymbol && MovementSymbol->Value && MovementSymbol->Value->Spline)
    MovementSpline = MovementSymbol->Value->Spline;
  ...
  if(PathSpline)
  {
    ...
    if(sege >= Para.Segments)
    {
      rangee -= (sege - (Para.Segments - 1));
      sege = Para.Segments - 1;
    }
