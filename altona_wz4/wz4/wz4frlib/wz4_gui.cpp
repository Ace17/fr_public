...
void Wz4SplineCed::OnDrag(const sWindowDrag& dd, const sRect& client)
{
  ...
  switch(DragMode)
  {
  case DM_SCROLL:       DragScroll(dd);
    break;
  case DM_ZOOM:         DragZoom(dd);
    break;
  case DM_HANDLE:       DragHandle(dd, 3);
    break;
  case DM_HANDLEX:      DragHandle(dd, 1);
    break;
  case DM_HANDLEY:      DragHandle(dd, 2);
    break;
  case DM_SCALE:        DragScale(dd, 3);
    break;
  case DM_SCALEX:       DragScale(dd, 1);
    break;
  case DM_SCALEY:       DragScale(dd, 2);
    break;
  case DM_FRAME:        DragFrame(dd);
    break;
  case DM_FRAMEADD:     DragFrame(dd, 1);
    break;
  case DM_INSERT:       DragInsert(dd);
    break;
  case DM_COPYCAM:      DragCopyCam(dd);
    break;
  case DM_SCRATCHSLOW:  App->TimelineWin->DragScratchIndirect(dd, 0x100);
    break;
  case DM_SCRATCHFAST:  App->TimelineWin->DragScratchIndirect(dd, 0x4000);
    break;
  case DM_TIME:         DragTime(dd);
    break;
  case DM_SCISSORALL:   DragScissor(dd, 0);
    break;
  case DM_SCISSORVIS:   DragScissor(dd, 1);
    break;
  }
    ...
    sFORALL(Doc->AllOps, op)
    {
      if(op->Class == cl || op->Class == cl2 || op->Class == mcl)
      {
        ...
        if((op->Class == cl && (((Wz4RenderParaClip*)op->EditData)->Flags & 0x0c) == 0) || op->Class == cl2)
        {
          ...
          if(c->Start + c->Length > tmax)
            tmax = c->Start + c->Length;
        }
...
void Wz4TimelineCed::OnDrag(const sWindowDrag& dd, const sRect& client)
{
  ...
  case DM_ZOOM:         DragZoom(dd);
    break;
  case DM_FRAME:        DragFrame(dd, 0);
    break;
  case DM_FRAMEADD:     DragFrame(dd, 1);
    break;
  case DM_HANDLE:       DragHandle(dd, 3);
    break;
  case DM_HANDLELEN:    DragHandle(dd, 4);
    break;
...
void Wz4TimelineCed::DragHandle(const sWindowDrag& dd, sDInt mode)
{
  ...
  case sDD_DRAG:
    sFORALL(Clips, clip)
    {
      ...
      {
        if(mode & 1)
          clip->Start = sClamp(clip->DragStart + dx, 0, 0x40000000);

        if(mode & 2)
          clip->Line = sClamp(clip->DragLine + dy, 0, 31);

        if(mode & 4)
          clip->Length = sClamp(clip->DragLength + dx, 0, 0x40000000);
...
// case DM_FRAMEADD:     DragFrame(dd,1); break;
  ...
  case DM_SELECT:       DragSelect(dd, 0);
    break;
  case DM_SET:          DragSelect(dd, 1);
    break;
  case DM_ACCENT:       DragSelect(dd, 2);
    break;
  }
