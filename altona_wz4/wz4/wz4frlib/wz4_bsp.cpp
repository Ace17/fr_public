...
const sChar* Wz4BSPGetErrorString(Wz4BSPError err)
{
  switch(err)
  {
  case WZ4BSP_OK:                   return L"No error";
    break;
  case WZ4BSP_NONPLANAR_INPUT:      return L"Nonplanar input (try increasing epsilon)";
    break;
  case WZ4BSP_NONPLANAR_POLY:       return L"Cannot produce planar polygon - increase epsilon.";
    break;
  case WZ4BSP_DEGENERATE_POLY:      return L"Unfixable degenerate polygon produced - increase epsilon.";
    break;
  case WZ4BSP_CANT_HIT_CLIP_PLANE:  return L"Cannot hit clip plane in SplitEdgeAlongPlane - increase epsilon.";
    break;
  case WZ4BSP_CANT_HIT_BOTH_PLANES: return L"Cannot hit both clip and poly plane in SplitEdgeAlongPlane - increase epsilon!";
    break;
  case WZ4BSP_ROUNDOFF_PROBLEM:     return L"Nonplanar polygons due to roundoff drift (try increasing epsilon)";
    break;
  case WZ4BSP_WHAT_THE_FUCK:        return L"\"What the fuck?\" error. Give a reproduce to ryg ASAP.";
    break;
...
static void SolveCoordFromPlane(sVector31& p, const sVector4& plane, sInt axis)
{
  switch(axis)
  {
  case 0: p.x = (-plane.w - p.y * plane.y - p.z * plane.z) / plane.x;
    break;
  case 1: p.y = (-plane.w - p.z * plane.z - p.x * plane.x) / plane.y;
    break;
  case 2: p.z = (-plane.w - p.x * plane.x - p.y * plane.y) / plane.z;
    break;
...
Wz4BSPError Wz4BSPPolyhedron::Split(const sVector4& plane, Wz4BSPPolyhedron& front, Wz4BSPPolyhedron& back, sF32 eps) const
{
  ...
  {
  case WZ4BSP_BACK:   back = *this;
    return WZ4BSP_OK;
  case WZ4BSP_ON: // fall-through
  case WZ4BSP_FRONT:  front = *this;
    return WZ4BSP_OK;
  default:            break;
  }
  ...
  for(sInt i = 0; i < Faces.GetCount(); i++)
  {
    ...
    if(error == WZ4BSP_OK)
    {
      ...
      if(frontFace)
        front.Faces.AddTail(frontFace);

      if(backFace)
        back.Faces.AddTail(backFace);
    }
  }
...
void Wz4BSPPolyhedron::GenMesh(Wz4Mesh* mesh, sF32 explode, const sVector30& shift) const
{
  ...
  sFORALL(Faces, face)
  {
    ...
    for(sInt i = 0; i < face->Count; i++)
    {
      ...
      switch(projAxis)
      {
      case 0: mv->U0 = mv->Pos.y;
        mv->V0 = mv->Pos.z;
        break;
      case 1: mv->U0 = mv->Pos.z;
        mv->V0 = mv->Pos.x;
        break;
      case 2: mv->U0 = mv->Pos.x;
        mv->V0 = mv->Pos.y;
        break;
      default: sVERIFYFALSE;
      }
...
Wz4BSPPolyClass Wz4BSPPolyhedron::Classify(const sVector4& plane, sF32 eps) const
{
  ...
  for(sInt i = 0; i < Faces.GetCount(); i++)
  {
    ...
    {
    case WZ4BSP_STRADDLING: nStraddling++;
      break;
    case WZ4BSP_BACK:       nBack++;
      break;
    case WZ4BSP_FRONT:      nFront++;
      break;
  ...
  for(sInt i = 0; i < splitFaces.GetCount(); i++)
  {
    ...
    case WZ4BSP_STRADDLING:
      ...
      if(ErrorCode == WZ4BSP_OK)
      {
        ...
        if(frontFace)
          frontSplit.AddTail(frontFace);

        if(backFace)
          backSplit.AddTail(backFace);
      }
...
Wz4BSPNode* Wz4BSP::MakeRandomSplitsR(Wz4BSPNode* node, sRandomMT& rand, const Wz4BSPPolyhedron& poly, sInt& maxSplits)
{
  ...
  if(node->Type == Wz4BSPNode::Inner)
  {
    ...
    if(!back.IsEmpty())
      node->Child[0] = MakeRandomSplitsR(node->Child[0], rand, back, maxSplits);

    if(!front.IsEmpty())
      node->Child[1] = MakeRandomSplitsR(node->Child[1], rand, front, maxSplits);
  }
