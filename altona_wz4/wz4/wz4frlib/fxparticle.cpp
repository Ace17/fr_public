...
static void FBMAddRef()
{
  ++FBMRefCount;
}

static void FBMRelease()
{
  if(!--FBMRefCount)
    FinalBoneMats.Reset();
}

static sMatrix34CM* FBMUse(sInt count)
{
  FinalBoneMats.Clear();
  return FinalBoneMats.AddMany(count);
}
...
void RNSprites::Render(Wz4RenderContext* ctx)
{
  ...
  if(ctx->IsCommonRendermode(zwrite))
  {
    ...
    case 0x0004: fade0col = 0x00000000;
      break;
    case 0x0005: fade0col = Para.Color & 0x00ffffff;
      break;
    }
...
void RNTrails::Prepare(Wz4RenderContext* ctx)
{
  ...
  if(PInfos[0].Alloc == 0 || TrailCount < 2)
    return;

  if(Mtrl)
    Mtrl->BeforeFrame(Para.LightEnv);
...
void RPFromMesh::Init(Wz4Mesh* mesh)
{
  ...
  if(zm * ym * xm == 0 || zm * ym * xm > 0x1000000)
    return;
