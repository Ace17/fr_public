...
const sU32 RNCubeTrees::CubeVertexDecl[] =
{
  sVF_POSITION, sVF_NORMAL, sVF_TANGENT | sVF_F4, sVF_UV0, 0
};
...
{
  if(!CubeVertexFormat)
    CubeVertexFormat = sCreateVertexFormat(CubeVertexDecl);
...
void RNCubeTrees::MakeBranch(sInt depth, sInt seed, sF32 time, sF32 width, sVector31 pos, sVector30 dir, sVector30Arg ref, sVertexStandard*& vp, sU16*& ip, sInt& ic, sInt& vc)
{
  ...
  sRandom rnd;
  rnd.Seed(Para.Seed + 100 * depth + seed);
    ...
    if(grow > seg && pos.y > (width + Para.CubeSize) && !(code & 3))
    {
      ...
      if(time2 > 0)
      {
        ...
        if(depth < 2)
        {
          ...
          {
            sVector30 dir2 = ref;
            sVector30 ref2 = ref % dir;
            ref2.Unit();

            if(sFAbs(dir2.y) >= 0.71f)
              sSwap(dir2, ref2); // branch goes to the sides
...
{
  if(TreeMtrl)
    TreeMtrl->BeforeFrame(Para.LightEnv);

  if(CubeMtrl)
    CubeMtrl->BeforeFrame(Para.LightEnv);
  ...
  if(nCubes)
  {
    ...
    {
      static const sInt codes[6][4] =
      {
        0, 1, 2, 3, 1, 5, 3, 7, 5, 4, 7, 6, 4, 0, 6, 2, 3, 7, 2, 6, 5, 1, 4, 0,
      };
      ...
      ns[0] = -m.k;
      ns[0].Unit();
      ts[0].Init(m.i.x, m.i.y, m.i.z, 1);
      ns[1] = m.i;
      ns[1].Unit();
      ts[1].Init(m.k.x, m.k.y, m.k.z, -1);
      ns[2] = -ns[0];
      ts[2].Init(-m.i.x, -m.i.y, -m.i.z, -1);
      ns[3] = -ns[1];
      ts[3].Init(-m.k.x, -m.k.y, -m.k.z, 1);
      ns[4] = m.j;
      ns[4].Unit();
      ts[4].Init(m.k.x, m.k.y, m.k.z, 1);
      ns[5] = -ns[4];
      ts[5].Init(-m.k.x, -m.k.y, -m.k.z, -1);
...
void RNCubeTrees::Render(Wz4RenderContext* ctx)
{
  ...
  {
    if(TreeMtrl->SkipPhase(ctx->RenderMode, Para.LightEnv & 15))
      return;
