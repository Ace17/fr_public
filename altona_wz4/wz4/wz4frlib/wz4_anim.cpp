...
void Wz4AnimKey::Serialize_(streamer& stream)
{
...
void Wz4AnimKey::Serialize(sWriter& stream)
{
  Serialize_(stream);
}

void Wz4AnimKey::Serialize(sReader& stream)
{
  Serialize_(stream);
}
...
void SerializeWz4Channel(sReader& stream, Wz4Channel*& chan)
{
  ...
  case sSerId::Wz4ChannelSpline:
    {
      Wz4ChannelSpline* c = new Wz4ChannelSpline;
      c->Serialize(stream);
      chan = c;
    }
    break;
...
template<class streamer>
void Wz4ChannelPerFrame::Serialize_(streamer& stream)
{
  ...
  if(version)
  {
    ...
    {
      if(stream.If(0))
        Scale = new sVector31[Keys];

      if(stream.If(0))
        Rot = new sQuaternion[Keys];

      if(stream.If(0))
        Trans = new sVector31[Keys];

      if(stream.If(0))
        User = new sVector31[Keys];
    }

    if(Scale)
      stream.ArrayF32((sF32*)Scale, Keys * 3);

    if(Rot)
      stream.ArrayF32((sF32*)Rot, Keys * 4);

    if(Trans)
      stream.ArrayF32((sF32*)Trans, Keys * 3);

    if(User)
      stream.ArrayF32((sF32*)User, Keys * 3);

    stream.Footer();
  }
}

void Wz4ChannelPerFrame::Serialize(sWriter& stream)
{
  Serialize_(stream);
}

void Wz4ChannelPerFrame::Serialize(sReader& stream)
{
  Serialize_(stream);
}
...
template<class streamer>
void Wz4ChannelSpline::Serialize_(streamer& stream)
{
...
void Wz4ChannelSpline::Serialize(sWriter& stream)
{
  Serialize_(stream);
}

void Wz4ChannelSpline::Serialize(sReader& stream)
{
  Serialize_(stream);
}
...
sBool Wz4ChannelSpline::Approximate(const Wz4ChannelPerFrame& target, sF32 err)
{
  ...
  if(target.Scale)
    ok &= Scale.FitToCurve((sVector30*)target.Scale, target.Keys, err, 1.0f);

  if(target.Trans)
    ok &= Trans.FitToCurve((sVector30*)target.Trans, target.Keys, err, 1.0f);
  ...
  if(Scale.IsConstant(err))
  {
    Scale.Evaluate((sVector30 &)Start.Scale, 0.0f);
    Scale.Clear();
  }

  if(Rot.IsConstant(err))
  {
    Rot.Evaluate(Start.Rot, 0.0f);
    Rot.Clear();
  }

  if(Trans.IsConstant(err))
  {
    Trans.Evaluate((sVector30 &)Start.Trans, 0.0f);
    Trans.Clear();
  }
...
void Wz4ChannelSpline::Evaluate(sF32 time, Wz4AnimKey& result)
{
  ...
  if(!Scale.IsEmpty())
    Scale.Evaluate((sVector30 &)result.Scale, time);

  if(!Rot.IsEmpty())
    Rot.Evaluate(result.Rot, time);

  if(!Trans.IsEmpty())
    Trans.Evaluate((sVector30 &)result.Trans, time);
}
...
template<class streamer>
void Wz4Skeleton::Serialize_(streamer& stream)
{
  Wz4AnimJoint* joint;
...
void Wz4Skeleton::Serialize(sWriter& stream)
{
  Serialize_(stream);
}

void Wz4Skeleton::Serialize(sReader& stream)
{
  Serialize_(stream);
}
  ...
  sFORALL(Joints, j)
  {
    if(j->Channel)
    {
      ...
      else
      {
        ...
        j->Channel->Evaluate(time2, key);
        key.ToMatrix(mat2);
        mat.i.Fade(fade, mat1.i, mat2.i);
        mat.i.Unit();
        mat.j.Fade(fade, mat1.j, mat2.j);
        mat.j.Unit();
        mat.k.Fade(fade, mat1.k, mat2.k);
        mat.k.Unit();
