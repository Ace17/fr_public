...
void __stdcall Bitmap_Inner(sU64* d, sU64* s, sInt count, sInt mode, sU64* x)
{
  ...
  case BI_ALPHA:
    {
      static const sALIGNED(sU16, maskc[8], 16) =
      {
...
void GenBitmap::GlowRect(sF32 cx, sF32 cy, sF32 rx, sF32 ry, sF32 sx, sF32 sy, sU32 color, sF32 alpha, sF32 power, sInt wrap, sInt bug)
{
  ...
  {
    if(cx + rx + sx > 1.0f)
      GlowRect(cx - 1.0f, cy, rx, ry, sx, sy, color, alpha, power, 2, bug);
  ...
  {
    if(cy + ry + sy > 1.0f)
      GlowRect(cx, cy - 1.0f, rx, ry, sx, sy, color, alpha, power, 0, bug);
...
void GenBitmap::Bump(GenBitmap* bb, sInt subcode, sF32 px, sF32 py, sF32 pz, sF32 da, sF32 db, sU32 _diff, sU32 _ambi, sF32 outer, sF32 falloff, sF32 amp, sU32 _spec, sF32 spow, sF32 samp)
{
  sInt xs, ys;
  sInt x, y;
  sU16* d, * b, * s;
...
void GenBitmap::Text(sF32 x, sF32 y, sF32 width, sF32 height, sU32 col, sU32 flags, sF32 lineskip, const sChar* text, const sChar* fontname)
{
  ...
  if(flags & 0x100)
    createflags |= sF2C_ITALICS;

  if(flags & 0x200)
    createflags |= sF2C_BOLD;

  if(flags & 0x400)
    createflags |= sF2C_SYMBOLS;
...
void GenBitmap::Bricks(sInt bmxs, sInt bmys, sInt color0, sInt color1, sInt colorf, sF32 ffugex, sF32 ffugey, sInt tx, sInt ty, sInt seed, sInt heads, sInt flags, sF32 side, sF32 colorbalance)
{
  sU64* d;
  sInt fx, fy;
  sInt fugex, fugey;
    ...
    for(sInt x = 0; x < XSize; x++)
    {
      ...
      if(flags & 8)
      {
        ...
        if(cells[by * tx + (bx + 0) % tx].Kopf)
          if(fx < fugex)
            f = 0x4000 * sMax(0, ((fx) - (fugex - bmx))) / bmx;

        if(cells[by * tx + (bx + 1) % tx].Kopf)
          if(0x4000 - fx < fugex)
            f = 0x4000 * sMax(0, ((0x4000 - fx) - (fugex - bmx))) / bmx;

        if(fy < fugey)
          f = sMin(f, 0x4000 * sMax(0, ((fy) - (fugey - bmy))) / bmy);

        if(0x4000 - fy < fugey)
          f = sMin(f, 0x4000 * sMax(0, ((0x4000 - fy) - (fugey - bmy))) / bmy);
      }
      else
      {
        f = 0x4000;
