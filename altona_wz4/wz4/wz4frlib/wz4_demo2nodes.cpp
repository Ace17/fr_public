...
{
  if(Active && Para.Enable)
    Wz4RenderNode::Transform(ctx, mat);
}

void RNClip::Prepare(Wz4RenderContext* ctx)
{
  if(Active && Para.Enable)
    Wz4RenderNode::Prepare(ctx);
}
...
{
  if(Active && Para.Enable && ActiveClip >= 0)
    Clips[ActiveClip].Child->Transform(ctx, mat);
}

void RNClipRand::Prepare(Wz4RenderContext* ctx)
{
  if(Active && Para.Enable && ActiveClip >= 0)
    Clips[ActiveClip].Child->Prepare(ctx);
}

void RNClipRand::Render(Wz4RenderContext* ctx)
{
  if(Active && Para.Enable && ActiveClip >= 0)
    Clips[ActiveClip].Child->Render(ctx);
}
  ...
  if((para->Flags & SSF_BoundMask) == SSF_BoundWrap) // loop spline
  {
    for(sInt i = 0; i < count; i++)
    {
      ...
      if(max >= 2)
      {
        ...
        k0 = (*Spline->Curves[i])[0];
        k0.Time += para->MaxTime;
        k1 = (*Spline->Curves[i])[1];
        k1.Time += para->MaxTime;
        k2 = (*Spline->Curves[i])[max - 2];
        k2.Time -= para->MaxTime;
        k3 = (*Spline->Curves[i])[max - 1];
        k3.Time -= para->MaxTime;
...
void RNLayer2D::Init()
{
  static sInt blend[16] =
  {
    sMB_OFF, sMB_PMALPHA, sMB_ADD, sMB_SUB, sMB_MUL, sMB_MUL2, sMB_ADDSMOOTH, sMB_ALPHA, sMBS_1 | sMBO_ADD | sMBD_SCI,
  ...
  if(Texture[0])
    Mtrl->Texture[0] = Texture[0]->Texture;

  if(Texture[1])
    Mtrl->Texture[1] = Texture[1]->Texture;
...
void RNLayer2D::Prepare(Wz4RenderContext* ctx)
{
  ...
  if(tex && Para.Aspect & 4)
    sizeY *= tex->SizeY / DocScreenY;
