...
void Wz4MtrlType_::Init()
{
  ...
  if(Wz4MtrlType == 0)      // this is not a derived class!
  {
    sU32 descw[] =
    {
      sVF_POSITION, sVF_NORMAL, sVF_COLOR0, 0
    };
    sU32 descwi[] =
    {
      sVF_POSITION, sVF_NORMAL, sVF_COLOR0, sVF_STREAM1 | sVF_UV5 | sVF_F4, sVF_STREAM1 | sVF_UV6 | sVF_F4, sVF_STREAM1 | sVF_UV7 | sVF_F4, 0
    };
    sU32 descwip[] =
    {
      sVF_POSITION, sVF_NORMAL, sVF_COLOR0, sVF_STREAM1 | sVF_UV5 | sVF_F4, sVF_STREAM1 | sVF_UV6 | sVF_F4, sVF_STREAM1 | sVF_UV7 | sVF_F4, sVF_STREAM1 | sVF_UV4 | sVF_F4, 0
    };

    sU32 descz[] =
    {
      sVF_POSITION, sVF_NORMAL, 0
    };
    sU32 desczb[] =
    {
      sVF_POSITION, sVF_NORMAL, sVF_BONEINDEX | sVF_I4, sVF_BONEWEIGHT | sVF_I4, 0
    };
    sU32 desczi[] =
    {
      sVF_POSITION, sVF_NORMAL, sVF_STREAM1 | sVF_UV5 | sVF_F4, sVF_STREAM1 | sVF_UV6 | sVF_F4, sVF_STREAM1 | sVF_UV7 | sVF_F4, 0
    };
    sU32 desczip[] =
    {
      sVF_POSITION, sVF_NORMAL, sVF_STREAM1 | sVF_UV5 | sVF_F4, sVF_STREAM1 | sVF_UV6 | sVF_F4, sVF_STREAM1 | sVF_UV7 | sVF_F4, sVF_STREAM1 | sVF_UV4 | sVF_F4, 0
    };
    ...
    sU32 desceb[] =
    {
      sVF_POSITION, sVF_BONEINDEX | sVF_I4, sVF_BONEWEIGHT | sVF_I4, 0
    };
    sU32 descei[] =
    {
      sVF_POSITION, sVF_STREAM1 | sVF_UV5 | sVF_F4, sVF_STREAM1 | sVF_UV6 | sVF_F4, sVF_STREAM1 | sVF_UV7 | sVF_F4, 0
    };
    sU32 desceip[] =
    {
      sVF_POSITION, sVF_STREAM1 | sVF_UV5 | sVF_F4, sVF_STREAM1 | sVF_UV6 | sVF_F4, sVF_STREAM1 | sVF_UV7 | sVF_F4, sVF_STREAM1 | sVF_UV4 | sVF_F4, 0
    };
    ...
    for(sInt i = 0; i < sRF_TOTAL; i++)
    {
      ...
      if(mtrl)
      {
        ...
        if((i & sRF_TARGET_MASK) == sRF_TARGET_ZNORMAL)
          zmode = 0;

        if((i & sRF_TARGET_MASK) == sRF_TARGET_ZONLY)
          zmode = 1;

        if((i & sRF_TARGET_MASK) == sRF_TARGET_DIST)
          zmode = 2;
  ...
  case sRF_TARGET_DIST:
    {
      ...
      {
      case sMTRL_CULLON: variant = det < 0 ? 2 : 1;
        break;
      case sMTRL_CULLINV: variant = det < 0 ? 1 : 2;
        break;
      }
  ...
  case sRF_TARGET_MAIN:
    {
      ...
      {
      default:
      case sRF_MATRIX_ONE:      shader = 0;
        break;
      case sRF_MATRIX_BONE:     shader = 1;
        break;
      case sRF_MATRIX_INSTANCE: shader = 2;
        break;
      case sRF_MATRIX_BONEINST: shader = 3;
        break;
      case sRF_MATRIX_INSTPLUS: shader = 4;
        break;
      }
      ...
      if(Tex[0])
        Shaders[shader]->Texture[0] = Tex[0]->Texture;

      if(Tex[1])
        Shaders[shader]->Texture[1] = Tex[1]->Texture;

      if(Tex[2])
        Shaders[shader]->Texture[2] = Tex[2]->Texture;
...
void SimpleMtrl::Prepare()
{
  ...
  *desc = 0;
  ...
  for(sInt i = 0; i < sCOUNTOF(Shaders); i++)
  {
    ...
    if(Tex[0])
      Shaders[i]->Texture[0] = Tex[0]->Texture;

    if(Tex[1])
      Shaders[i]->Texture[1] = Tex[1]->Texture;

    if(Tex[2])
      Shaders[i]->Texture[2] = Tex[2]->Texture;
...
sVertexFormatHandle* SimpleMtrl::GetFormatHandle(sInt flags)
{
  ...
  case sRF_TARGET_MAIN | sRF_MATRIX_BONEINST: return Formats[3];
    break;
  case sRF_TARGET_MAIN | sRF_MATRIX_INSTPLUS: return Formats[4];
    break;
...
void SimpleMtrl::Serialize_(streamer& s)
{
...
void RNSimpleMtrlEnv::Render(Wz4RenderContext* ctx)
{
  ...
  if(Para.Headlights & 0x0001)
    d0 = d0 * ctx->View.Camera;

  if(Para.Headlights & 0x0010)
    d1 = d1 * ctx->View.Camera;

  if(Para.Headlights & 0x0100)
    d2 = d2 * ctx->View.Camera;

  if(Para.Headlights & 0x1000)
    d3 = d3 * ctx->View.Camera;
  ...
  cbv->lc[0].InitColor(Para.Color0);
  cbv->lc[0] = cbv->lc[0] * Para.Color0Amp;
  cbv->lc[1].InitColor(Para.Color1);
  cbv->lc[1] = cbv->lc[1] * Para.Color1Amp;
  cbv->lc[2].InitColor(Para.Color2);
  cbv->lc[2] = cbv->lc[2] * Para.Color2Amp;
  cbv->lc[3].InitColor(Para.Color3);
  cbv->lc[3] = cbv->lc[3] * Para.Color3Amp;
  ...
  if(clipbits & 0x10)
    clipbits |= 0x08;     // bit 4 (0x10) is set to calculate clip3 fogging
...
sVertexFormatHandle* CustomMtrl::GetFormatHandle(sInt flags)
{
  switch(flags & sRF_TOTAL)
  {
  case sRF_TARGET_MAIN | sRF_MATRIX_ONE: return Format;
    break;
  default: return Wz4MtrlType->GetDefaultFormat(flags);
  }
}
