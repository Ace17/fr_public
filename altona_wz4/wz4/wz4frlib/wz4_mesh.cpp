...
sBool Wz4MeshVertex::operator == (const Wz4MeshVertex& v) const
{
  ...
  if(Tangent.x != v.Tangent.x)
    return 0;

  if(Tangent.y != v.Tangent.y)
    return 0;

  if(Tangent.z != v.Tangent.z)
    return 0;
  ...
  for(sInt i = 0; i < 4; i++)
  {
    ...
    if(Index[i] >= 0 && Weight[i] != v.Weight[i])
      return 0;
  }
...
void Wz4Mesh::Serialize_(streamer& s)
{
  sInt version = s.Header(sSerId::Wz4Mesh, 2);
  version;
    ...
    if(!(SaveFlags & 1))
    {
      if(s.IsReading())
      {
        ...
        if(s.If(0))
        {
          switch(s.PeekHeader())
          {
          case sSerId::Wz4SimpleMtrl: c.Mtrl = new SimpleMtrl;
            break;
...
void Wz4Mesh::Serialize(sWriter& stream)
{
  Serialize_(stream);
}

void Wz4Mesh::Serialize(sReader& stream)
{
  Serialize_(stream);
}
...
void Wz4Mesh::MergeVertices()
{
  ...
  if(0)
    sDPrintF(L"optimize mesh: %k -> %k vertices\n", max, vc);
...
void Wz4Mesh::CalcNormals(sInt* map, sBool onlyselected)
{
  ...
  if(onlyselected)
    oldnormals = new sVector30[Vertices.GetCount()];

  sFORALL(Vertices, vp)
  {
    if(onlyselected)
      oldnormals[_i] = vp->Normal;
...
void Wz4Mesh::CalcTangents(sInt* map, sBool onlyselected)
{
  ...
  if(onlyselected)
    oldtangents = new sVector30[Vertices.GetCount()];
...
void Wz4Mesh::Heal(Wz4Mesh* reference, sInt flags, sF32 posThres, sF32 normThres)
{
  ...
  sFORALL(Vertices, v)
  {
    ...
    {
      if((v->Pos - vref->Pos).LengthSq() > posThres)
        continue;

      if((v->Normal ^ vref->Normal) < normThres)
        continue;

      if(flags & 1)
        v->Normal = vref->Normal;

      if(flags & 2)
        v->Tangent = vref->Tangent;
    ...
    for(sInt ei = 0; ei < isl->NumEdges; ei++)
    {
      ...
      for(sInt j = 0; j < steps + 1; j++)
      {
        ...
        for(sInt k = 0; k < 2; k++)
        {
          ...
          {
            if(va.Index[i] >= 0)
              v[j * 2 + k].AddWeight(va.Index[i], (1.0f - fade) * va.Weight[i]);

            if(vb.Index[i] >= 0)
              v[j * 2 + k].AddWeight(vb.Index[i], fade * vb.Weight[i]);
          }
...
sBool Wz4Mesh::DivideInChunksR(Wz4MeshFace* mf, sInt mfi, Wz4MeshFaceConnect* conn)
{
  ...
  for(sInt i = 0; i < mf->Count; i++)
  {
    ...
    else if(Vertices[mf->Vertex[i]].SelectTemp != mf->Temp)
      return 0;
  }
...
struct WireFormat
{
  ...
  void Init(const sVector31& pos, sU32 col)
  {
    Pos = pos;
    Normal.Init(0, 0, 0);
    Color = col;
  ...
  void Init(const sVector31& pos, const sVector30& norm, sU32 col)
  {
    Pos = pos;
    Normal = norm;
    Color = col;
...
void Wz4Mesh::ChargeSolid(sInt flags)
{
  ...
  if(Clusters.GetCount() == 0)
    return;

  if(Clusters[0]->Geo[geoindex])
    return;
...
HFONT __stdcall CreateFontW(sInt height, sInt width, sInt escape, sInt orient, sInt weight, sInt italic, sInt underline, sInt strikeOut, sU32 charSet, sU32 outPrecision, sU32 clipPrecision, sU32 quality, sU32 pitchAndFamily, const sChar* face);
HFONT __stdcall SelectObject(HDC hDC, HFONT hFont);
sU32 __stdcall GetGlyphOutlineW(HDC hDC, sU32 nChar, sU32 fuFormat, GLYPHMETRICS* lpgm, sU32 cjBuffer, void* buffer, const MAT2* lpmat2);
...
void Wz4Mesh::MakeText(const sChar* text, const sChar* font, sF32 height, sF32 extrude, sF32 maxErr, sInt flags)
{
  ...
  for(sInt chr = 0; text[chr]; chr++)
  {
    ...
    sFORALL(tempmesh->Vertices, vertex)
    {
      ...
      if(flags & 16)
      {
        sSwap(vertex->Pos.x, vertex->Pos.z);
        vertex->Pos.x *= -1;
        sSwap(vertex->Normal.x, vertex->Normal.z);
        vertex->Normal.x *= -1;
        sSwap(vertex->Tangent.x, vertex->Tangent.z);
        vertex->Normal.x *= -1;
      }
...
sInt Wz4Mesh::SplitEdgeAlongPlane(sInt va, sInt vb, const sVector4& plane)
{
  ...
  v->Normal.Fade(t, a.Normal, b.Normal);
  v->Normal.Unit();
  v->Tangent.Fade(t, a.Tangent, b.Tangent);
  v->Tangent.Unit();
  v->BiSign = a.BiSign;
...
void Wz4Mesh::SplitAlongPlane(const sVector4& plane)
{
  ...
  for(sInt i = 0; i < faceCount; i++)
  {
    ...
    if(frontCount >= 3)
      SplitGenFace(i, frontVert, frontCount, sTRUE);

    if(backCount >= 3)
      SplitGenFace(i, backVert, backCount, frontCount < 3);
  }
}
