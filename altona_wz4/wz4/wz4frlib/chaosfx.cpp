...
void RNCubeExample::MakeCube()
{
  ...
  vp->Init(-1, 1, -1, 0, 0, -1, 1, 0);
  vp++; // 3
  vp->Init(1, 1, -1, 0, 0, -1, 1, 1);
  vp++; // 2
  vp->Init(1, -1, -1, 0, 0, -1, 0, 1);
  vp++; // 1
  vp->Init(-1, -1, -1, 0, 0, -1, 0, 0);
  vp++; // 0
  ...
  vp->Init(-1, -1, -1, 0, -1, 0, 1, 0);
  vp++; // 0
  vp->Init(1, -1, -1, 0, -1, 0, 1, 1);
  vp++; // 1
  vp->Init(1, -1, 1, 0, -1, 0, 0, 1);
  vp++; // 5
  vp->Init(-1, -1, 1, 0, -1, 0, 0, 0);
  vp++; // 4

  vp->Init(1, -1, -1, 1, 0, 0, 1, 0);
  vp++; // 1
  vp->Init(1, 1, -1, 1, 0, 0, 1, 1);
  vp++; // 2
  vp->Init(1, 1, 1, 1, 0, 0, 0, 1);
  vp++; // 6
  vp->Init(1, -1, 1, 1, 0, 0, 0, 0);
  vp++; // 5
...
void RNRibbons::Render(Wz4RenderContext* ctx)
{
  ...
  if(ctx->IsCommonRendermode())
  {
    ...
    {
      if(MtrlEx->SkipPhase(ctx->RenderMode, Para.LightEnv))
        return;
    }
