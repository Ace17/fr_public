...
void XSIMaterial::Connect(sPoolString shader, sPoolString para, sPoolString value)
{
  if(shader == L"xsi_tc3")
  {
    ...
    if(para == L"para_tex00_image")
      stage = 0;

    if(para == L"para_tex01_image")
      stage = 1;

    if(para == L"para_tex02_image")
      stage = 2;

    if(para == L"para_tex03_image")
      stage = 3;
...
void XSIMaterial::TSpace(sPoolString shader, sPoolString para, sPoolString mesh, sPoolString value)
{
  if(shader == L"xsi_tc3")
  {
    ...
    if(para == L"para_tex00_texturespace_id")
      stage = 0;

    if(para == L"para_tex01_texturespace_id")
      stage = 1;

    if(para == L"para_tex02_texturespace_id")
      stage = 2;

    if(para == L"para_tex03_texturespace_id")
      stage = 3;
      ...
      while(Scan.Token != '}' && !Scan.Errors)
      {
        if(Scan.IfName(L"XSI_ShaderInstanceData"))
        {
          ...
          for(sInt i = 0; i < max; i++)
          {
            ...
            if(goodshader)
              mtrl->TSpace(shader, para, model, value);
          }
