  ...
  case sRF_TARGET_MAIN:
    {
      ...
      {
      default:
      case sRF_MATRIX_ONE:      sh = 0;
        break;
      case sRF_MATRIX_BONE:     sh = 1;
        break;
      case sRF_MATRIX_INSTANCE: sh = 2;
        break;
      case sRF_MATRIX_BONEINST: sh = 3;
        break;
      }
...
void RoveMtrl::Prepare()
{
  ...
  *desc = 0;
  ...
  for(sInt i = 0; i < 3; i++)
  {
    ...
    if(Tex[0])
      Shaders[i]->Texture[1] = Tex[0]->Texture;

    if(Tex[1])
      Shaders[i]->Texture[2] = Tex[1]->Texture;
...
void RNRoveMtrlEnv::Render(Wz4RenderContext* ctx)
{
  ...
  col0.InitColor(Para.Color0);
  col0 *= Para.Color0Amp;
  col1.InitColor(Para.Color1);
  col1 *= Para.Color1Amp;
  col2.InitColor(Para.Color2);
  col2 *= Para.Color2Amp;
  col3.InitColor(Para.Color3);
  col3 *= Para.Color3Amp;
