...
{
  sClear(UVMatrix0);
  UVMatrix0[0][0] = UVMatrix0[1][1] = 1;
  sClear(UVMatrix1);
  UVMatrix1[0][0] = UVMatrix1[1][1] = 1;
  UVMatrix2.Init();
  DetailMode = 0;
}
...
// sU32 *data=buffer;
  ...
  if((DetailMode & 0x0f) == 3 && !Texture[0])
    DetailMode &= ~0x0f;    // detail bump

  if((DetailMode & 0x0f) == 4 && !Texture[3])
    DetailMode &= ~0x0f;    // gloss
  ...
  if(DetailMode & 0x200)
  {
    if(fmt->Has(sVF_BONEINDEX))
      vs |= Wz4CShaderVSPermMask_SkinEnable;
  ...
  else if(Texture[0] == 0 && Texture[3] == 0 && Texture[4] == 0 && Texture[6] == 0 && Texture[1] != 0)
  {
    if(Texture[2])
    {
      ...
      {
      case 1: ps |= Wz4LShaderPSPerm_TexDetailMul;
        break;
      case 2: ps |= Wz4LShaderPSPerm_TexDetailAdd;
        break;
      }
...
// if(Texture[5]) ps |= Wz4LShaderPSPermMask_TexEnvi;
    ...
    if(fmt->Has(sVF_BONEINDEX))
      vs |= Wz4LShaderVSPermMask_SkinEnable;
  ...
  else
  {
    ...
    if(Texture[3])
      ps |= Wz4ShaderPSPermMask_TexSpecularCube;

    if(Texture[4])
      ps |= Wz4ShaderPSPermMask_TexDiffuseCube;
...
// if(Texture[5]) ps |= Wz4ShaderPSPermMask_TexEnvi;
    ...
    if(Texture[2])
    {
      switch(DetailMode & 0x0f)
      {
      case 1: ps |= Wz4ShaderPSPerm_TexDetailMul;
        break;
      case 2: ps |= Wz4ShaderPSPerm_TexDetailAdd;
        break;
      case 3: ps |= Wz4ShaderPSPerm_TexDetailBump;
        break;
      case 4: ps |= Wz4ShaderPSPerm_TexDetailSpecMul;
        break;
      }
      switch(DetailMode & 0xf0)
      {
    ...
    if(fmt->Has(sVF_BONEINDEX))
      vs |= Wz4ShaderVSPermMask_SkinEnable;
...
void Wz4Material::Serialize_(streamer& stream, sTexture2D* shadow)
{
  ...
  if(version)
  {
    ...
    for(sInt i = 0; i < maxtex; i++)
    {
      ...
      switch(n)
      {
      case 1: stream.OnceRef(tex2d);
        break;
      case 2: stream.OnceRef(texcube);
        break;
      }
    ...
    for(sInt i = 0; i < 4; i++)
      stream.U8(Material->FuncFlags[i]);

    for(sInt i = 0; i < 6; i++)
      stream.U8(Material->StencilOps[i]);
...
void Wz4Material::Serialize(sWriter& stream, sTexture2D* shadow)
{
  Serialize_(stream, 0);
}

void Wz4Material::Serialize(sReader& stream, sTexture2D* shadow)
{
  Serialize_(stream, shadow);
}
