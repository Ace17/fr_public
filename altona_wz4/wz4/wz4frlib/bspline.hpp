...
template<class T>
struct sBSplineHelper
{
  static T PostWeight(const T& in)
  {
    return in;
  }
...
template<>
struct sBSplineHelper<sF32>
{
  static sF32 PostWeight(sF32 in)
  {
    return in;
  }
...
template<>
struct sBSplineHelper<sQuaternion>
{
  static sQuaternion PostWeight(const sQuaternion& in)
  {
    sQuaternion t = in;
    t.Unit();
    return t;
  ...
  static sF32 PrescaleError(sF32 err, const sQuaternion* samples, sInt count)
  {
    return err * err * 4.0f;
  }
};
...
public:
  ...
  sBSpline<T> & operator = (const sBSpline<T>& x)
  {
    T temp = x;
    Swap(temp);
  }

  void Swap(sBSpline<T>& x)
  {
    sSwap(NumKnots, x.NumKnots);
    sSwap(Knots, x.Knots);
    sSwap(ControlPoints, x.ControlPoints);
  }
  ...
  sInt GetNumKnots() const
  {
    return NumKnots;
  }

  sInt GetNumControlPoints() const
  {
    return NumKnots ? NumKnots - sBSPLINE_ORDER : 0;
  }

  const sF32* GetKnots() const
  {
    return Knots;
  }

  const T* GetControlPoints() const
  {
...
template<class T>
sBSpline<T>::sBSpline(const sBSpline<T>& x)
{
  NumKnots = x.NumKnots;
...
template<class T>
template<class streamer>
void sBSpline<T>::Serialize_(streamer& stream)
{
...
template<class T>
sBool sBSpline<T>::FitToCurve(const T* samples, sInt nSamples, sF32 maxRelativeError, sF32 normalizedLength)
{
...
template<class T>
sBool sBSpline<T>::IsConstant(sF32 relErrorThreshold) const
{
