...
public:
  ...
  sInt GetFreq(sInt symbol)
  {
    return Freq[symbol * 2 + 1];
  }

  sInt GetCumFreq(sInt symbol)
  {
    return Freq[symbol * 2 + 0];
  }
  ...
  sF32 GetBits(sInt symbol)
  {
    return -sFLog(1.0f * GetFreq(symbol) / GetTotFreq()) * 1.4427f;
  }
...
sF32 NRVPackerBackEnd::MatchLen(sU32 offs, sU32 len, sU32 PO)
{
  ...
  else
  {
    ...
    switch(Variant)
    {
    case 0: b = 1 + 8 + SizePrefix11(1 + (offs >> 8));
      break;
    case 1: b = 1 + 9 + SizePrefix12(1 + (offs >> 7));
      break;
    case 2: b = 1 + 8 + SizePrefix12(1 + (offs >> 7));
      break;
    }
  }
  switch(Variant)
  {
  case 0: b += 2 + (len >= 4 ? SizePrefix11(len - 4) : 0);
    break;
  case 1: b += (len >= 4 ? SizePrefix11(len - 4) : 0);
    break;
  case 2:
...
void NRVPackerBackEnd::EncodeEnd(sF32* outProfile)
{
  ...
  switch(Variant)
  {
  case 0: EncodePrefix11(0x1000000);
    break;
  case 1:
  case 2: EncodePrefix12(0x1000000);
    break;
  }
...
// if(log.PrintFs) goto ende;
  ...
  {
    data[0] = string;
    string += file->NameSize;
    data[1] = offset;
    offset += sAlign(file->PackedSize, 16);
