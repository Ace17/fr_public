...
public:
  ...
  inline unsigned int GetNbOfVertices()
  {
    return nbvertices;
  }

  inline unsigned int GetNbOfIndices()
  {
    return nbindices;
  }
  ...
  inline unsigned int GetNbOfTex()
  {
    return nbtex;
  }
  ...
  inline float* GetVNormals()
  {
    return vnormals;
  }               /* Get normals of vertices */

  inline float* GetFNormals()
  {
    return fnormals;
  }               /* Get normals of faces */

  inline float* GetSTs(unsigned int tex)
  {
    return vst[tex];
  }
    ...
    case LW_DO_TAG('V', 'M', 'A', 'P'):
      {
        ...
        if(LW_ReadUInt(pos, in) == LW_DO_TAG('T', 'X', 'U', 'V'))
          ...
          {
            char acTxt[64];
            int iLen = LW_ReadString(pos, in, acTxt);
            iLen;
