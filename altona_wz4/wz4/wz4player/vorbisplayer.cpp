...
public:
  ...
  sInt GetLatency() const
  {
    return BufferSize;
  }

  sInt GetChannels() const
  {
    return Channels;
  }
...
private:
  ...
  static void ThreadProxy(sThread*, void* p)
  {
    reinterpret_cast<bSoundOutput*>(p)->ThreadFunc();
  }
  ...
  void ThreadFunc()
  {
    ...
    if(clen1)
      sSetMem(cptr1, 0, clen1);

    if(clen2)
      sSetMem(cptr2, 0, clen2);
    ...
    for(;;)
    {
      ...
      if(todo >= 32)
      {
        ...
        if(clen1)
          ClipTo(cptr1, RenderBuffer, clen1 / BytesPerSample);

        if(clen2)
          ClipTo(cptr2, RenderBuffer + Channels * (clen1 / BytesPerSample), clen2 / BytesPerSample);
...
struct bMusicPlayer::VerySecret : protected bRenderer
{
  ...
  sInt GetSamples(sF32* buffer, sInt nFloats)
  {
    ...
    {
      if(V2Length && V2Decoded >= V2Length)
        return 0;
...
bMusicPlayer::bMusicPlayer(sBool seekable)
{
  p = new VerySecret(seekable);
}

bMusicPlayer::~bMusicPlayer()
{
  delete p;
}

void bMusicPlayer::Init(const sChar* filename)
{
  p->Init(filename);
}

// void bMusicPlayer::Init(void *ptr, sDInt size) { p->Init(ptr,size);  }
void bMusicPlayer::Exit()
{
  p->Exit();
}

void bMusicPlayer::Play()
{
  p->Play();
}

void bMusicPlayer::Pause()
{
  p->Pause();
}

void bMusicPlayer::Seek(sF32 time)
{
  p->Seek(time);
}

void bMusicPlayer::Skip(sInt samples)
{
  sAtomicAdd(&p->SkipSamples, samples);
}

void bMusicPlayer::SetVolume(sF32 vol, sF32 fadeduration)
{
  p->SetVolume(vol, fadeduration);
}

void bMusicPlayer::SetLoop(sBool loop)
{
  p->Loop = loop;
}

sBool bMusicPlayer::IsPlaying()
{
  return p->IsPlaying();
}

sF32 bMusicPlayer::GetPosition()
{
  return p->GetPosition();
}

sBool bMusicPlayer::IsLoaded()
{
  return p->Buffer != 0;
}
