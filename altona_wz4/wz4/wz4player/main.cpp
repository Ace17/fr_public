...
void sMain()
{
  ...
  else
  {
    ...
    if(opt.SiteId)
    {
      ...
      static const sChar* siteids[] =
      {
        L"twitter", L"facebook", L"myspace", L"digg", L"stumbleupon"
      };
    ...
    if(!bOpenSelector(selsetup, Selection))
      return;
  }

  if(Selection.Mode.Flags & sSM_FULLSCREEN)
