...
sBool sHTTPClient::Connect(const sChar* url, ConnMethod method)
{
  ...
  for(;;)
  {
    ...
    for(;;)
    {
      ...
      if(llen < 1)
        break; // no line found (more data needed)
      ...
      {
        if(sCmpMem(lp, L"HTTP/1.", 7))
          goto headererr;

        lp += 9; // skip "HTTP/1.x "

        if(!sScanInt(lp, RetCode))
          goto headererr;
...
sBool sHTTPClient::Read(void* buffer, sDInt size, sDInt& read)
{
  ...
  if(ChunkLeft >= 0)
    while(size > 0 && read == 0) // chunked transfer
    {
      sDInt read2;
...
sInt sHTTPClient::GetSize()
{
  return IsConnected() ? Size : -1;
}
...
void sHTTPServer::ParseRequestLine(Connection* c)
{
  ...
  {
    // parse method
    if(sScanMatch(cp, L"GET "))
      c->Method = CM_GET;
    else if(sScanMatch(cp, L"HEAD "))
      c->Method = CM_HEAD;
    else if(sScanMatch(cp, L"POST "))
      c->Method = CM_POST;
    else
    {
...
void sHTTPServer::StartServing(Connection* c)
{
  ...
  if(c->RetCode >= 200)
  {
    ...
    else
    {
      ...
      if(c->Hndl)
        c->Hndl->GetAdditionalHeaders(sGetAppendDesc(str));
...
sBool sHTTPServer::Run(sThread* t)
{
  ...
  while(HostSocket.IsConnected() && (!t || t->CheckTerminate()))
  {
    ...
    {
      if(c->State == CS_GETREQUEST)
        readset[nreads++] = c->Socket;
          ...
          for(sInt ip = 0; ip < read; ip++)
          {
            ...
            if((c->Method != CM_POST) || (!c->HeadersDone))
            {
              ...
              if(ch == 10)
              {
                ...
                while(op >= 0 && c->RequestLine[op] < 32)
                  c->RequestLine[op--] = 0;
...
void sHTTPServer::SimpleHandler::WriteHTMLHeader(const sChar* title, const sChar* stylesheet, const sChar* addheader)
{
  ...
  if(title)
    PrintF(L"  <title>%s</title>\n", title);

  if(stylesheet)
    PrintF(L"  <link rel=\"stylesheet\" href=\"%s\">\n", stylesheet);
...
void sHTTPServer::DataPacket::Parse()
{
  ...
  while((linelen > 0) && (*(p + linelen - 1) < 32))
    linelen--;
