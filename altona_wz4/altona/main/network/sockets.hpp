...
struct sIPAddress
{
  ...
  sIPAddress(sU8 a, sU8 b, sU8 c, sU8 d)
  {
    Addr[0] = a;
    Addr[1] = b;
    Addr[2] = c;
  ...
  sIPAddress(const sIPAddress& a)
  {
    sCopyMem(this, &a, sizeof(sIPAddress));
  }

  bool operator == (const sIPAddress& a) const
  {
    return !sCmpMem(this, &a, sizeof(sIPAddress));
  }
  ...
  const sChar* ToString(const sStringDesc& dest) const
  {
    sSPrintF(dest, L"%d.%d.%d.%d", Addr[0], Addr[1], Addr[2], Addr[3]);
    return dest.Buffer;
  }
};
...
public:
  ...
  template<typename T>
  inline sBool ReadAll(T& var)
  {
    return ReadAll(&var, sizeof(var));
  }

  template<typename T>
  inline sBool WriteAll(const T& var)
  {
    return WriteAll(&var, sizeof(var));
  }

  sBool ReadAll(const sStringDesc& s)
  {
    sInt len = 0;
    ReadAll(len);

    if(len > s.Size)
      sFatal(L"ReadAll(string) buffer overflow");

    return ReadAll(s.Buffer, len * sizeof(sChar));
  }

  sBool WriteAll(const sStringDesc& s)
  {
    sInt len = sGetStringLen(s.Buffer) + 1;
    WriteAll(len);
    return WriteAll(s.Buffer, len * sizeof(sChar));
  }

  // check for read/write errors
  sBool CheckReadError() const
  {
    return TransferError & 1;
  }

  sBool CheckWriteError() const
  {
    return TransferError & 2;
  }

  sBool CheckError() const
  {
...
public:
  ...
  sBool Write(const void*, sDInt, sDInt &)
  {
    return sFALSE;
  }

  sBool Read(void*, sDInt, sDInt &)
  {
    return sFALSE;
  }
...
public:
  ...
  // convenience
  template<typename T>
  inline sBool Write(const sIPAddress& destaddr, sIPPort destport, const T& var)
  {
    return Write(destaddr, destport, &var, sizeof(var));
  }

  template<typename T>
  inline sBool Broadcast(sIPPort destport, const T& var)
  {
    return Broadcast(destport, &var, sizeof(var));
  }
