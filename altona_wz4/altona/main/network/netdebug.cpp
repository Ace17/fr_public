...
public:
  ...
  sHTTPServer::HandlerResult WriteDocument(const sChar* URL)
  {
    ...
    if(mode == MODE_LOC)
    {
      ...
      if(!memtype)
        PrintF(L"<td align=right>Type</td>");
    ...
    else if(mode == MODE_DESC)
    {
      ...
      switch(sortby)
      {
      case 0: sHeapSortUp(descs, &sMemoryLeakDesc::String);
        break;
      case 1: sHeapSortUp(descs, &sMemoryLeakDesc::Hash);
        break;
      case 2: sHeapSortUp(descs, &sMemoryLeakDesc::RefCount);
        break;
      }
...
struct PerfStats : public sHTTPServer::SimpleHandler
{
  ...
  struct SortEntry
  {
    ...
    bool operator > (const SortEntry& b) const
    {
      return sCmpStringI(Name, b.Name) > 0;
    }
  };
