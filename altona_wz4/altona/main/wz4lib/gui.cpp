...
void MainWindow::MainInit()
{
  ...
  PageListWin->InitWire(L"PageList");
  TreeViewWin = new WinTreeView;
  TreeViewWin->InitWire(L"TreeView");
  ParaWin[0] = new WinPara;
  ParaWin[0]->InitWire(L"Para");
  ParaWin[1] = new WinPara;
  ParaWin[1]->InitWire(L"AnimPara");
  ViewWin[0] = new WinView;
  ViewWin[0]->InitWire(L"View0");
  ViewWin[1] = new WinView;
  ViewWin[1]->InitWire(L"View1");
  ViewWin[2] = new WinView;
  ViewWin[2]->InitWire(L"View2");
  StackWin = new WinStack;
  StackWin->InitWire(L"Page");
  StoreBrowserWin = new WinStoreBrowser;
  StoreBrowserWin->InitWire(L"StoreBrowser");
  PopupTextWin = new WinPopupText;
  PopupTextWin->InitWire(L"PopupText");
  ...
  OpListWin = new WinOpList;
  OpListWin->InitWire(L"OpList");
// OpListWin->AddBorder(new sListWindow2Header(OpListWin));
  Status = new sStatusBorder;
  sWire->AddWindow(L"Status", Status);

  CustomWin = new WinCustom;
  sWire->AddWindow(L"Custom", CustomWin);
...
void MainWindow::SetShellSwitches()
{
  ...
  sFORALL(Doc->ScriptDefines, sd)
  {
    ...
    switch(sd->Mode)
    {
    case 1: gh.String(&sd->StringValue);
      break;
    case 2: gh.Int(&sd->IntValue, -0x8000000LL, 0x7ffffff);
      break;
    case 3: gh.Float(&sd->FloatValue, -65536.0f * 65536, 65536.0f * 65536);
      break;
    }
        ...
        case ScriptTypeInt:
          ...
          else
          {
            for(sInt i = 0; i < sp->Count; i++)
            {
              ...
              if(sp->GuiExtras > 0)
              {
                const static sChar* formats[9] =
                {
                  0, L"%01x", L"%02x", L"%03x", L"%04x", L"%05x", L"%06x", L"%07x", L"%08x"
                };
...
void WinPara::FileLoadDialog(sDInt offset)
{
  if(Op && Op->Class->ParaStrings > offset)
  {
    ...
    if(sSystemOpenFileDialog(L"Select File", Op->Class->FileInFilter.IsEmpty() ? L"" : Op->Class->FileInFilter, sSOF_LOAD, FileDialogBuffer))
    {
      ...
      {
        if(FileDialogBuffer[len] == '/' || FileDialogBuffer[len] == '\\')
          len++;
...
void WinStack::OnPaint2D()
{
  ...
  if(Page)
  {
    ...
    sFORALL(Page->Ops, op)
    {
      if((op->Class->Flags & wCF_COMMENT))
      {
        static sU32 colors[8] =
        {
          0xffffff, 0xff0000, 0xffff00, 0x00ff00, 0x00ffff, 0x0000ff, 0xff00ff, 0x000000
        };
        MakeRect(r, op);
        r.Extend(4);
        sRect rr(r);
    ...
    sFORALL(Page->Ops, op)
    {
      if((op->Class->Flags & wCF_LOAD) && op->Links.GetCount() == 1 && op->Links[0].Link)
      {
        ...
        if(target->Page == Page)
        {
          ...
          for(sInt i = 0; i < 5; i++)
          {
            static const sInt jx[5] =
            {
              0, -1, 0, 1, 0
            };
            static const sInt jy[5] =
            {
              0, 0, -1, 0, 1
            };
            sInt x = jx[i];
