...
sInt BinaryOp(sInt token, sInt& pri, sInt& mode)
{
  ...
  case TOK_SHIFTL:        pri = 4;
    mode = 0x0001;
    return SC_SHIFTL;
  case TOK_SHIFTR:        pri = 4;
    mode = 0x0001;
    return SC_SHIFTR;
  case TOK_ROLL:          pri = 4;
    mode = 0x0001;
  ...
  case TOK_LOGAND:        pri = 10;
    mode = 0x0401;
    return SC_BINAND;
  case TOK_LOGOR:         pri = 11;
    mode = 0x0401;
    return SC_BINOR;
...
void ScriptCompiler::PrintExpr(Expression* expr)
{
  ...
  {
  case SC_ADD:    PrintFunc(L"+", expr->a, expr->b);
    break;
  case SC_SUB:    PrintFunc(L"-", expr->a, expr->b);
    break;
  case SC_MUL:    PrintFunc(L"*", expr->a, expr->b);
    break;
  case SC_DIV:    PrintFunc(L"/", expr->a, expr->b);
    break;
  case SC_MOD:    PrintFunc(L"%", expr->a, expr->b);
    break;
  case SC_DOT:    PrintFunc(L"\x00ba", expr->a, expr->b);
    break;

  case SC_NEG:    PrintFunc(L"-", expr->a);
    break;
  case SC_FTOI:   PrintFunc(L"ftoi ", expr->a);
    break;
  case SC_ITOF:   PrintFunc(L"itof ", expr->a);
    break;
  case SC_FTOC:   PrintFunc(L"ftoc ", expr->a);
    break;
  case SC_CTOF:   PrintFunc(L"ctof ", expr->a);
    break;

  case SC_SHIFTL: PrintFunc(L">>", expr->a, expr->b);
    break;
  case SC_SHIFTR: PrintFunc(L"<<", expr->a, expr->b);
    break;
  case SC_ROLL:   PrintFunc(L">>>", expr->a, expr->b);
    break;
  case SC_ROLR:   PrintFunc(L"<<<", expr->a, expr->b);
    break;

  case SC_NOT:    PrintFunc(L"!", expr->a);
    break;
  case SC_NOTNOT: PrintFunc(L"?", expr->a);
    break;
  case SC_LOGAND: PrintFunc(L"&&", expr->a, expr->b);
    break;
  case SC_LOGOR:  PrintFunc(L"||", expr->a, expr->b);
    break;
  case SC_BINAND: PrintFunc(L"&", expr->a, expr->b);
    break;
  case SC_BINOR:  PrintFunc(L"|", expr->a, expr->b);
    break;
  case SC_BINXOR: PrintFunc(L"^", expr->a, expr->b);
    break;

  case SC_EQ:     PrintFunc(L"==", expr->a, expr->b);
    break;
  case SC_NE:     PrintFunc(L"!=", expr->a, expr->b);
    break;
  case SC_GT:     PrintFunc(L">", expr->a, expr->b);
    break;
  case SC_GE:     PrintFunc(L">=", expr->a, expr->b);
    break;
  case SC_LT:     PrintFunc(L"<", expr->a, expr->b);
    break;
  case SC_LE:     PrintFunc(L"<=", expr->a, expr->b);
    break;

  case SC_ABS:    PrintFunc2(L"abs", expr);
    break;
  case SC_SIGN:   PrintFunc2(L"sign", expr);
    break;
  case SC_MAX:    PrintFunc2(L"max", expr);
    break;
  case SC_MIN:    PrintFunc2(L"min", expr);
    break;
  case SC_SIN:    PrintFunc2(L"sin", expr);
    break;
  case SC_COS:    PrintFunc2(L"cos", expr);
    break;
  case SC_SIN1:   PrintFunc2(L"sin1", expr);
    break;
  case SC_COS1:   PrintFunc2(L"cos1", expr);
    break;
  case SC_TAN:    PrintFunc2(L"tan", expr);
    break;
  case SC_ATAN:   PrintFunc2(L"atan", expr);
    break;
  case SC_ATAN2:  PrintFunc2(L"atan2", expr);
    break;
  case SC_SQRT:   PrintFunc2(L"sqrt", expr);
    break;
  case SC_POW:    PrintFunc2(L"pow", expr);
    break;
  case SC_EXP:    PrintFunc2(L"exp", expr);
    break;
  case SC_LOG:    PrintFunc2(L"log", expr);
    break;
  case SC_SMOOTHSTEP: PrintFunc2(L"smoothstep", expr);
    break;
  case SC_RAMPUP: PrintFunc2(L"rampup", expr);
    break;
  case SC_RAMPDOWN: PrintFunc2(L"rampdown", expr);
    break;
  case SC_TRIANGLE: PrintFunc2(L"triangle", expr);
    break;
  case SC_PULSE: PrintFunc2(L"pulse", expr);
    break;
  case SC_EXPEASE: PrintFunc2(L"expease", expr);
    break;
  case SC_FADEINOUT: PrintFunc2(L"fadeinout", expr);
    break;
  case SC_NOISE:  PrintFunc2(L"noise", expr);
    break;
  case SC_PERLINNOISE: PrintFunc2(L"perlinnoise", expr);
    break;

  case SC_CLAMP:  PrintFunc2(L"clamp", expr);
    break;
  case SC_LENGTH: PrintFunc2(L"length", expr);
    break;
  case SC_NORMALIZE:  PrintFunc2(L"normalize", expr);
    break;
  case SC_MAP:    PrintFunc2(L"map", expr);
    break;
  ...
  case SC_ASSIGN: PrintFunc(L"=", expr->b, expr->a);
    break;
...
void ScriptCompiler::ConstFold(Expression* expr)
{
  ...
  if(expr->a && expr->a->Kind != SC_LITERAL)
    return;

  if(expr->b && expr->b->Kind != SC_LITERAL)
    return;

  if(expr->c && expr->c->Kind != SC_LITERAL)
    return;

  if(expr->d && expr->d->Kind != SC_LITERAL)
    return;
  ...
  if(expr->Count > 0)
  {
    ...
    if(type == ScriptTypeInt)  // int
    {
      ...
      {
        if(expr->a)
          a = ((sInt*)expr->a->Literal)[i];

        if(expr->b)
          b = ((sInt*)expr->b->Literal)[i];
        switch(expr->Kind)
        {
        case SC_ADD: val[i] = a + b;
          break;
        case SC_SUB: val[i] = a - b;
          break;
        case SC_MUL: val[i] = a * b;
          break;
        case SC_DIV:
        ...
        case SC_SHIFTL: val[i] = a << b;
          break;
        case SC_SHIFTR: val[i] = a >> b;
          break;
        case SC_ROLL:   val[i] = (a << b) | (a >> (32 - b));
          break;
        case SC_ROLR:   val[i] = (a >> b) | (a << (32 - b));
          break;

        case SC_LOGAND: val[i] = a && b;
          break;
        case SC_LOGOR:  val[i] = a || b;
          break;
        case SC_BINAND: val[i] = a & b;
          break;
        case SC_BINOR:  val[i] = a | b;
          break;
        case SC_BINXOR: val[i] = a ^ b;
          break;

        case SC_EQ: val[i] = (a == b);
          break;
        case SC_NE: val[i] = (a != b);
          break;
        case SC_GT: val[i] = (a > b);
          break;
        case SC_GE: val[i] = (a >= b);
          break;
        case SC_LT: val[i] = (a < b);
          break;
        case SC_LE: val[i] = (a <= b);
          break;

        case SC_NEG:  val[i] = -a;
          break;
        case SC_FTOI: val[i] = sInt(((sF32*)expr->a->Literal)[i]);
          break;
        case SC_NOT:  val[i] = !a;
          break;
        case SC_NOTNOT: val[i] = !!a;
          break;
    ...
    else if(type == ScriptTypeFloat)    // float
    {
      ...
      {
        if(expr->a)
          a = ((sF32*)expr->a->Literal)[i];

        if(expr->b)
          b = ((sF32*)expr->b->Literal)[i];
        switch(expr->Kind)
        {
        case SC_ADD: val[i] = a + b;
          break;
        ...
        case SC_MUL: val[i] = a * b;
          break;
        case SC_DIV: val[i] = a / b;
          break;
        case SC_MOD: val[i] = sFMod(a, b);
          break;
        ...
        case SC_ITOF: val[i] = sF32(((sS32*)expr->a->Literal)[i]);
          break;
