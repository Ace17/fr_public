...
void WinView::SetPaintInfo(sBool render3D, sViewport* view, sMaterialEnv* env)
{
  ...
  if(render3D)
  {
    ...
    {
    case 0: SideSpeed = ForeSpeed = 0.001f;
      break;
    case 1: SideSpeed = ForeSpeed = 0.01f;
      break;
    case 2: SideSpeed = ForeSpeed = 0.1f;
      break;
    case 3: SideSpeed = ForeSpeed = 1.0f;
      break;
    }
    ...
    if(hnd)
    {
      ...
      case wHM_RAY:
        ...
        sFORALL(pi.SelectedHandles, ip)
        {
          ...
          if(hnd2->x)
            tag2->Drag.x = *hnd2->x - DragStart.x;

          if(hnd2->y)
            tag2->Drag.y = *hnd2->y - DragStart.y;

          if(hnd2->z)
            tag2->Drag.z = *hnd2->z - DragStart.z;
        }
        break;
...
void WinView::CmdQuantizeCamera()
{
  ...
  if(ax > ay && ax > az)
    v.Init(sSign(v.x), 0, 0);

  if(ay > az)
    v.Init(0, sSign(v.y), 0);
  else
    v.Init(0, 0, sSign(v.z));
