...
GroupType* GroupType::Make(wObject* e1, wObject* e2)
{
  ...
  grp->Members.AddTail(e1);

  if(e1)
    e1->AddRef();

  grp->Members.AddTail(e2);
...
void ScreenshotProxyType_::Show(wObject* obj, wPaintInfo& pi)
{
  ...
  if(proxy->Root)
  {
    ...
    if(movie)
    {
      ...
      for(digits = 1; d < framecount; digits++)
        d *= 10;
    ...
    for(sInt frame = 0; frame < framecount; frame++)
    {
      if(movie)
      {
        ...
        if(shot)
          sDPrintF(L"rendering frame %d at %dms(beat %08x\n", frame, ms, beat);

        if(ProgressPaintFunc)
          ProgressPaintFunc(frame, framecount);
      }
...
void BitmapAtlas::Serialize_(streamer& s)
{
  sInt version = s.Header(sSerId::Wz4BitmapAtlas, 2);
  version;
...
void BitmapAtlas::Serialize(sWriter& stream)
{
  Serialize_(stream);
}

void BitmapAtlas::Serialize(sReader& stream)
{
  Serialize_(stream);
}
...
template<class streamer>
void Texture2D::Serialize_(streamer& stream)
{
  ...
  if(!oldformat)
  {
    sInt version = stream.Header(sSerId::Wz4Texture2D, 1);
    version;
  }
...
void Texture2D::Serialize(sWriter& stream)
{
  Serialize_(stream);
}

void Texture2D::Serialize(sReader& stream)
{
  Serialize_(stream);
}
...
template<class streamer>
void TextureCube::Serialize_(streamer& stream)
{
  if(stream.IsReading())
    Cache = new sImageData;
  else
...
void TextureCube::Serialize(sWriter& stream)
{
  Serialize_(stream);
}

void TextureCube::Serialize(sReader& stream)
{
  Serialize_(stream);
}
...
sInt UnitTest::Test(sImage& img, const sChar* filename, sInt flags)
{
  ...
  switch(platform)
  {
  case wTOP_DirectX9:  path.AddPath(L"directx9");
    break;
  case wTOP_DirectX11: path.AddPath(L"directx11");
    break;
  default:             path.AddPath(L"unknwon");
    break;
  }
