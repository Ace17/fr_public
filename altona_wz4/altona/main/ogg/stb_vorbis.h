...
// WARNING! THIS FILE WAS EDITED BY dierk.chaos.ohlerich
// YOU CAN FIND ALL MY EDITS BY SEARCHING FOR 'CHAOS'
// I BASICALLY DISABLED SOME PARTS THAT I DONT NEED
...
extern stb_vorbis* stb_vorbis_open_pushdata(unsigned char* datablock, int datablock_length_in_bytes, int* datablock_memory_consumed_in_bytes, int* error, stb_vorbis_alloc* alloc_buffer);
...
extern int stb_vorbis_decode_frame_pushdata(stb_vorbis* f, unsigned char* datablock, int datablock_length_in_bytes, int* channels,             // place to write number of float * buffers
...
extern stb_vorbis* stb_vorbis_open_memory(unsigned char* data, int len, int* error, stb_vorbis_alloc* alloc_buffer);
// create an ogg vorbis decoder from an ogg vorbis stream in memory (note
// this must be the entire stream!). on failure, returns NULL and sets *error

#ifndef STB_VORBIS_NO_STDIO
extern stb_vorbis* stb_vorbis_open_filename(char* filename, int* error, stb_vorbis_alloc* alloc_buffer);
// create an ogg vorbis decoder from a filename via fopen(). on failure,
// returns NULL and sets *error (possibly to VORBIS_file_open_failure).

extern stb_vorbis* stb_vorbis_open_file(FILE* f, int close_handle_on_close, int* error, stb_vorbis_alloc* alloc_buffer);
...
extern stb_vorbis* stb_vorbis_open_file_section(FILE* f, int close_handle_on_close, int* error, stb_vorbis_alloc* alloc_buffer, unsigned int len);
// create an ogg vorbis decoder from an open FILE *, looking for a stream at
...
{
  if(f->alloc.alloc_buffer)
    return; // do nothing; setup mem is not a stack
...
static void setup_temp_free(vorb* f, void* p, size_t sz)
{
  if(f->alloc.alloc_buffer)
  {
    f->temp_offset += int((sz + 3) & ~3);         // CHAOS: REMOVED WARNING
    return;
  }
...
static void decode_residue(vorb* f, float* residue_buffers[], int ch, int n, int rn, uint8* do_not_decode)
{
  ...
  if(rtype == 2 && ch != 1)
  {
// int len = ch * n;       // CHAOS: unused variable
    for(j = 0; j < ch; ++j)
      if(!do_not_decode[j])
        break;
...
void dct_iv_slow(float* buffer, int n)
{
  ...
  float x[2048];
  int i, j;
// int n2 = n >> 1, nmask = (n << 3) - 1;        // CHAOS: unused variable
  int nmask = (n << 3) - 1;
...
{
// int k_off = -8;      // CHAOS: unused variable
  int a_off = base_n >> 3;
  float A2 = A[0 + a_off];
  float* z = e + i_off;
  float* base = z - 16 * n;
...
// int n3_4 = n - n4, ld;       // CHAOS: unused variable
  int ld;
...
stb_vorbis* stb_vorbis_open_pushdata(unsigned char* data, int data_len, // the memory available for decoding
