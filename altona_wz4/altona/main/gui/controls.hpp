...
public:
  ...
  template<class Type>
  void AddMultiChoice(sArray<Type*>& list, sInt Type::* offset)
  {
    Type* t;
    sFORALL(list, t) AddMultiChoice(&(t->*offset));
  }
...
public:
  ...
  {
    InitString(String);
    InitValue(val, min, max, step, colptr);
    Format = L"%f";
    MakeBuffer(1);
  }
