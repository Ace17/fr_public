...
protected:
  ...
  virtual sListWindowTreeInfo<sObject*>* GetTreeInfo(sObject* obj)
  {
    return 0;
  }
...
public:
  virtual sBool GetSelectStatus(sInt n)
  {
    return 0;
  }

  virtual sInt GetLastSelect()
  {
    return 0;
  }
  ...
  template<class T>
  sStaticListWindow(sArray<T*>* a)
  {
    Array = (sArray<sObject*>*)a;
    Construct();
  }
...
// sListWindow2Field *AddField(const sChar *label,sInt flags,sInt width,const sChar *sObject::*ref); // const string
  ...
  template<int n>
  sListWindow2Field* AddField(const sChar* label, sInt flags, sInt width, sMemberPtr<sString<n>> ref)
  {
    return AddField(label, flags, width, sMemberPtr<sChar>(ref.Offset), n);
  }
...
protected:
  ...
  void SetSelectStatus(sInt n, sInt select)
  {
  ...
  {
    if((!Array && Selected >= 0) || (Array && Selected >= Array->GetCount()))
    {
      Selected = -1;
      SelectMsg.Post();
    }
...
public:
  sSingleListWindow(sArray<Type*>* a) : sStaticListWindow(a)
  {
    Selected = -1;
  }
  ...
  Type* GetSelected()
  {
    return Selected >= 0 && Selected < Array->GetCount() ? (Type*)(*Array)[Selected] : 0;
  }

  void SetSelected(sInt n)
  {
    if(n == -1)
      ClearSelectStatus();
    else
  ...
  void SetSelected(Type* t)
  {
    SetSelected(sFindIndex(*Array, t));
  }
};
...
public:
  sBool GetSelectStatus(sInt n)
  {
    return Member.Ref((Type*)((*Array)[n]));
  }

  sInt GetLastSelect()
  {
    return (Cursor >= 0 && Cursor < Array->GetCount()) ? Cursor : -1;
  }
  ...
  void SetSelectStatus(sInt n, sInt select)
  {
    ...
    if(select != Member.Ref((Type*)((*Array)[n])))
    {
      Member.Ref((Type*)((*Array)[n])) = select;
      Update();
      SelectMsg.Post();
    }
  ...
  {
    sObject* t;
    sFORALL(*Array, t) Member.Ref((Type*)t) = 0;
    Update();
    SelectMsg.Post();
  }
...
public:
  sSingleTreeWindow(sArray<Type*>* a, sListWindowTreeInfo<Type*> Type::* treeinfo)
    : sSingleListWindow<Type>(a)
  {
    TreeInfoMember = treeinfo;
  }
};
...
public:
  sMultiTreeWindow(sArray<Type*>* a, sMemberPtr<sInt> select, sMemberPtr<sListWindowTreeInfo<Type*>> treeinfo)
    : sMultiListWindow<Type>(a, select)
  {
    TreeInfoMember = treeinfo;
  }
};
