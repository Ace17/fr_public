...
sBool s3DWindow::OnKey(sU32 key)
{
  ...
  if(QuakeMode)
  {
    ...
    case 'A' | sKEYQ_BREAK:
    case 'a' | sKEYQ_BREAK: QuakeMask &= ~1;
      use = 1;
      break;
    case 'D' | sKEYQ_BREAK:
    case 'd' | sKEYQ_BREAK: QuakeMask &= ~2;
      use = 1;
      break;
    case 'S' | sKEYQ_BREAK:
    case 's' | sKEYQ_BREAK: QuakeMask &= ~4;
      use = 1;
      break;
    case 'W' | sKEYQ_BREAK:
    case 'w' | sKEYQ_BREAK: QuakeMask &= ~8;
      use = 1;
      break;
    case 'Q' | sKEYQ_BREAK:
    case 'q' | sKEYQ_BREAK: QuakeMask &= ~16;
      use = 1;
      break;
    case 'E' | sKEYQ_BREAK:
    case 'e' | sKEYQ_BREAK: QuakeMask &= ~32;
      use = 1;
      break;
    }
  }
...
void s3DWindow::OnDrag(const sWindowDrag& dd)
{
  if(QuakeMode)
  {
    sInt mx, my;
    sGetMouseHard(mx, my);
    RotY += (mx - OldMouseHardX) * 0.002f;
    OldMouseHardX = mx;
    RotX += (my - OldMouseHardY) * 0.002f;
    OldMouseHardY = my;
