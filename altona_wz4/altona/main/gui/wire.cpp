...
// CurrentToolName = wc->CurrentTool ? wc->CurrentTool->Text : L""
    ...
    sFORALL(wc->Shortcuts, sc)
    {
      if(sc->Sets & win->WireSets)
      {
        ...
        if(sc->Qual & QUAL_ALLSHIFT)
        {
          if(sc->Key != (key & ~(sKEYQ_SHIFT | sKEYQ_CTRL | sKEYQ_ALT | sKEYQ_ALTGR)))
            ok = 0;
        }
        else
        {
        ...
        if(sc->Tool != wc->CurrentTool)
          ok = 0;

        if((sc->Qual & QUAL_HIT) && hit != sc->Hit)
          ok = 0;

        if((sc->Qual & QUAL_MISS) && hit)
          ok = 0;

        if((sc->Qual & QUAL_DOUBLE) && !(dd.Flags & sDDF_DOUBLECLICK))
          ok = 0;
...
// sInt other = 0;
  ...
  if(hascursor)
    mf->AddHeader(L"Cursor", 4);

  if(hasextra[0])
    mf->AddHeader(L"Extra", 8);

  if(hasextra[1])
    mf->AddHeader(L"Extra(2)", 12);

  if(hasextra[2])
    mf->AddHeader(L"Extra(3)", 16);

  if(hasdrag)
    mf->AddHeader(L"Mouse", 20);

  if(hastools)
    mf->AddHeader(L"Tools", 24);
  ...
  sFORALL(form->Shortcuts, sc)
  {
    if(sc->Sets & sets)
    {
      ...
      if(sc->Qual & QUAL_MISS)
        qual |= 0x02000000;

      if(sc->Qual & QUAL_DOUBLE)
        qual |= 0x04000000;
...
void sWireMasterWindow::_Key(sU32& key, sInt& qual, sInt& hit)
{
  ...
  else if(Scan->Token == sTOK_NAME)
  {
    if(Scan->Name == L"UP")
      key = sKEY_UP;
    else if(Scan->Name == L"DOWN")
      key = sKEY_DOWN;
    else if(Scan->Name == L"LEFT")
      key = sKEY_LEFT;
    else if(Scan->Name == L"RIGHT")
      key = sKEY_RIGHT;
    else if(Scan->Name == L"HOME")
      key = sKEY_HOME;
    else if(Scan->Name == L"END")
      key = sKEY_END;
    else if(Scan->Name == L"PAGEUP")
      key = sKEY_PAGEUP;
    else if(Scan->Name == L"PAGEDOWN")
      key = sKEY_PAGEDOWN;
    else if(Scan->Name == L"INSERT")
      key = sKEY_INSERT;
    else if(Scan->Name == L"DELETE")
      key = sKEY_DELETE;
    else if(Scan->Name == L"BACKSPACE")
      key = sKEY_BACKSPACE;
    else if(Scan->Name == L"TAB")
      key = sKEY_TAB;
    else if(Scan->Name == L"ENTER")
      key = sKEY_ENTER;
    else if(Scan->Name == L"ESCAPE")
      key = sKEY_ESCAPE;
    else if(Scan->Name == L"LMB")
      key = KEY_LMB;
    else if(Scan->Name == L"RMB")
      key = KEY_RMB;
    else if(Scan->Name == L"MMB")
      key = KEY_MMB;
    else if(Scan->Name == L"X4")
      key = KEY_X4;
    else if(Scan->Name == L"X5")
      key = KEY_X5;
    else if(Scan->Name == L"PAUSE")
      key = sKEY_PAUSE;
    else if(Scan->Name == L"MENU")
      key = sKEY_WINM;
    else if(Scan->Name == L"DROPFILES")
    {
      key = sKEY_DROPFILES;
      sEnableFileDrop(sTRUE);
    }
    else if(Scan->Name == L"WHEELUP")
      key = sKEY_WHEELUP;
    else if(Scan->Name == L"WHEELDOWN")
      key = sKEY_WHEELDOWN;
    else if(Scan->Name == L"NULL")
      key = KEY_NULL;
    else if(Scan->Name == L"HOVER")
      key = KEY_HOVER;

    else if(Scan->Name == L"F1")
      key = sKEY_F1;
    else if(Scan->Name == L"F2")
      key = sKEY_F2;
    else if(Scan->Name == L"F3")
      key = sKEY_F3;
    else if(Scan->Name == L"F4")
      key = sKEY_F4;
    else if(Scan->Name == L"F5")
      key = sKEY_F5;
    else if(Scan->Name == L"F6")
      key = sKEY_F6;
    else if(Scan->Name == L"F7")
      key = sKEY_F7;
    else if(Scan->Name == L"F8")
      key = sKEY_F8;
    else if(Scan->Name == L"F9")
      key = sKEY_F9;
    else if(Scan->Name == L"F10")
      key = sKEY_F10;
    else if(Scan->Name == L"F11")
      key = sKEY_F11;
    else if(Scan->Name == L"F12")
      key = sKEY_F12;
...
void sWireMasterWindow::_Popup()
{
  ...
  if(Scan->Token != '}')
    Scan->Error(L"only one window at top level. use dummy splitter");
  ...
  if(Scan->Token == '[' || Scan->Token == '.')
  {
    ...
    if(Scan->Token == '[')
    {
      ...
      if(Scan->Token == sTOK_NAME)
      {
        ...
        if(!inst)
          Scan->Error(L"could not find instance <%s> in form <%s>", name, form->Symbol);
      }
      else
      {
        sInt i = Scan->ScanInt();
...
// sInt hit = 0;
  ...
  if(!raw.Form.IsEmpty())
  {
    ...
    if(!raw.InstanceName.IsEmpty())
    {
      ...
      if(!inst)
        Scan->Error(L"could not find instance <%s> in form <%s>", raw.InstanceName, form->Symbol);
    }
...
void sWireMasterWindow::_Window()
{
  ...
  if(wc == 0)
    Scan->Error(L"class not found");

  if(wc->IsInScreen)
    Scan->Error(L"first define windows, then use in screens!");
  ...
  if(Scan->Token == sTOK_STRING)
  {
    if(windows.GetCount() >= 2)
      Scan->Error(L"can't name multiple windows");

    Scan->ScanString(text);

    if(wc->Text != L"")
      Scan->Error(L"window defined twice");
