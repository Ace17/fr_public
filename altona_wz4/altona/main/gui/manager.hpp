...
public:
  ...
  {
    RecCalcSize(w);
  } // calculate required size of a window subtree, even before it gets connected to the whole thing
  ...
  void Notify(const Type& Val)
  {
    Notify(&Val, sizeof(Val));
  }

  void CommandToAll(sInt cmd, sWindow* w = 0);
  void GetMousePos(sInt& x, sInt& y)
  {
    x = DragData.MouseX;
    y = DragData.MouseY;
  }
