...
void sColorGradient::Serialize_(streamer& s)
{
...
void sColorGradient::Serialize(sReader& stream)
{
  Serialize_(stream);
}

void sColorGradient::Serialize(sWriter& stream)
{
  Serialize_(stream);
}
...
void sColorPickerWindow::MakeGui()
{
  ...
  gh.Group(L"RGB");
  gh.Label(L"Red");
  gh.Float(&Red, 0, 1, step);
  gh.Label(L"Green");
  gh.Float(&Green, 0, 1, step);
  gh.Label(L"Blue");
  gh.Float(&Blue, 0, 1, step);

  if(AlphaEnable)
  {
    gh.ChangeMsg = sMessage(this, &sColorPickerWindow::ChangeRGB);
    gh.Label(L"Alpha");
    gh.Float(&Alpha, 0, 1, step);
  }

  gh.Group(L"HSV");
  gh.ChangeMsg = sMessage(this, &sColorPickerWindow::ChangeHSV);
  gh.Label(L"Hue");
  gh.Float(&Hue, -360, 720, 1.0f);
  gh.Label(L"Saturation");
  gh.Float(&Sat, 0, 1, step);
  gh.Label(L"Value");
  gh.Float(&Value, 0, 1, step);
