...
struct sWindowNotify              // update when something in range changes
{
  ...
  sBool Hit(const void* s, const void* e) const
  {
  ...
  void Add(const sWindowNotify& n)
  {
    if(End == 0)
    {
      *this = n;
...
struct sWindowMessage
{
  ...
  template<class T>
  sWindowMessage(void(T::* f)(sDInt), sDInt c)
  {
    Func = (sCommandFunc)f;
    Code = c;
  }
...
public:
  ...
  void AddNotify(const Type& Val)
  {
    AddNotify(&Val, sizeof(Val));
  }
