  ...
  while(!Scan.Errors && Scan.Token != sTOK_END)
  {
    if(Scan.IfToken(TOK_DEFINE))
    {
      ...
      else if(Scan.IfName(L"L"))
      {
        ...
        if(symbol == L"sCONFIG_CODEROOT")
        {
          used = 1;

          if(CodeRoot_Windows.IsEmpty())
            CodeRoot_Windows = name;

          if(CodeRoot_Linux.IsEmpty())
            CodeRoot_Linux = name;
        }

        if(symbol == L"sCONFIG_CODEROOT_WINDOWS")
        {
          used = 1;
          CodeRoot_Windows = name;
        }

        if(symbol == L"sCONFIG_CODEROOT_LINUX")
        {
          used = 1;
          CodeRoot_Linux = name;
        }

        if(symbol == L"sCONFIG_DATAROOT")
        {
          used = 1;
          DataRoot = name;
        }

        if(symbol == L"sCONFIG_MP_TEMPLATES")
        {
          used = 1;
          MakeprojectTemplate = name;
        }

        if(symbol == L"sCONFIG_CONFIGFILE")
        {
          used = 1;
          ConfigFile = name;
        }

        if(symbol == L"sCONFIG_TOOLBIN")
        {
          used = 1;
          ToolBin = name;
        }

        if(symbol == L"sCONFIG_INTERMEDIATEROOT")
        {
          used = 1;
          IntermediateRoot = name;
        }

        if(symbol == L"sCONFIG_OUTPUTROOT")
        {
          used = 1;
          OutputRoot = name;
        }

        if(symbol == L"sCONFIG_VSVERSION")
        {
          used = 1;
          VSVersion = name;
        }
