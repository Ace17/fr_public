...
sINLINE typename Range::ValueType& sPopHead(Range& r)
{
  typename Range::ValueType& item = r.GetHead();
  r.RemHead();
  return item;
}

template<typename Range>
sINLINE typename Range::ValueType& sPopTail(Range& r)
{
  typename Range::ValueType& item = r.GetTail();
  r.RemTail();
  return item;
}
...
public:
  ...
  sINLINE sBool IsEmpty() const
  {
    return Begin == End;
  }

  sINLINE sInt GetCount() const
  {
    return (sInt)(End - Begin);
  }
  ...
  sINLINE T & operator [] (sInt pos) const
  {
    return Begin[pos];
  }

  sINLINE sBool operator == (const sArrayRange& x)
  {
  ...
  sINLINE void RemHead()
  {
    ++Begin;
  }

  sINLINE void RemTail()
  {
    --End;
  }

  sINLINE ThisType Slice(sInt first, sInt last) const
  {
    return ThisType(Begin + first, Begin + last);
  }

  sINLINE DirectType GetDirect() const
  {
    return Begin;
  }

  // Swap isn't part of the range definition, but always good to have.
  sINLINE void Swap(sArrayRange<T>& b)
  {
    sSwap(Begin, b.Begin);
    sSwap(End, b.End);
  }
...
{
  typedef T ValueType;
  static sINLINE sArrayRange<T> Get(T x[N])
  {
    return sArrayRange<T>(x, x + N);
  }

  static sINLINE sArrayRange<const T> Get(const T x[N])
  {
    return sArrayRange<T>(x, x + N);
  }
};
...
template<typename T>
sINLINE sArrayRange<typename sAlgoImpl::GetAll<T>::ValueType> sAll(T& x)
{
  return sAlgoImpl::GetAll<T>::Get(x);
}

template<typename T>
sINLINE sArrayRange<const typename sAlgoImpl::GetAll<T>::ValueType> sAll(const T& x)
{
  return sAlgoImpl::GetAll<T>::Get(x);
}

template<template<typename> class ArrayType, typename T>
sINLINE sArrayRange<T> sAll(ArrayType<T>& x)
{
  T* ptr = x.GetData();
  return sArrayRange<T>(ptr, ptr + x.GetCount());
}

template<template<typename> class ArrayType, typename T>
sINLINE sArrayRange<const T> sAll(const ArrayType<T>& x)
{
  const T* ptr = x.GetData();
  return sArrayRange<const T>(ptr, ptr + x.GetCount());
}

template<template<typename, int> class ArrayType, typename T, int size>
sINLINE sArrayRange<T> sAll(ArrayType<T, size>& x)
{
  T* ptr = x.GetData();
  return sArrayRange<T>(ptr, ptr + x.GetCount());
}
...
template<template<typename, int> class ArrayType, typename T, int size>
sINLINE sArrayRange<const T> sAll(const ArrayType<T, size>& x)
{
  const T* ptr = x.GetData();
  return sArrayRange<const T>(ptr, ptr + x.GetCount());
}
...
// work plain C-arrays as well as sArray and all variants thereof.
template<typename T>
sINLINE sArrayRange<typename sAlgoImpl::GetAll<T>::ValueType> sAllMax(T& x)
{
  return sAlgoImpl::GetAll<T>::Get(x);
}

template<typename T>
sINLINE sArrayRange<const typename sAlgoImpl::GetAll<T>::ValueType> sAllMax(const T& x)
{
  return sAlgoImpl::GetAll<T>::Get(x);
}

template<template<typename> class ArrayType, typename T>
sINLINE sArrayRange<T> sAllMax(ArrayType<T>& x)
{
  T* ptr = x.GetData();
  return sArrayRange<T>(ptr, ptr + x.GetSize());
}

template<template<typename> class ArrayType, typename T>
sINLINE sArrayRange<const T> sAllMax(const ArrayType<T>& x)
{
  const T* ptr = x.GetData();
  return sArrayRange<const T>(ptr, ptr + x.GetSize());
}

template<template<typename, int> class ArrayType, typename T, int size>
sINLINE sArrayRange<T> sAllMax(ArrayType<T, size>& x)
{
  T* ptr = x.GetData();
  return sArrayRange<T>(ptr, ptr + x.GetSize());
}
...
template<template<typename, int> class ArrayType, typename T, int size>
sINLINE sArrayRange<const T> sAllMax(const ArrayType<T, size>& x)
{
  const T* ptr = x.GetData();
  return sArrayRange<const T>(ptr, ptr + x.GetSize());
}
...
template<typename T>
sINLINE sArrayRange<typename sAlgoImpl::GetAll<T>::ValueType> sAllResize(T& x, sInt newCount)
{
  return sAlgoImpl::GetAll<T>::Get(x);
}

template<template<typename> class ArrayType, typename T>
sINLINE sArrayRange<T> sAllResize(ArrayType<T>& x, sInt newCount)
{
  x.Resize(newCount);
  T* ptr = x.GetData();
  return sArrayRange<T>(ptr, ptr + x.GetSize());
...
template<template<typename, int> class ArrayType, typename T, int size>
sINLINE sArrayRange<T> sAllResize(ArrayType<T, size>& x, sInt newCount)
{
  x.Resize(newCount);
  T* ptr = x.GetData();
  return sArrayRange<T>(ptr, ptr + x.GetSize());
...
class sDListRange
{
  ...
  sINLINE const sDNode* GetNode(const T* e) const
  {
    return &(e->*Offset);
  }

  sINLINE const T* GetType(const sDNode* n) const
  {
    return (const T*)(((sU8*)n) - sOFFSETOMP((T*)0, Offset));
  }

  sINLINE sDNode* GetNode(T* e)
  {
    return &(e->*Offset);
  }

  sINLINE T* GetType(sDNode* n)
  {
    return (T*)(((sU8*)n) - sOFFSETOMP((T*)0, Offset));
  }
...
public:
  ...
  sDListRange() : Begin(0), End(0)
  {
  } // empty range

  sDListRange(T* beg, T* end) : Begin(beg), End(end)
  {
  } // beg inclusive, end exclusive

  sDListRange(sDList<T, Offset>& x)
  {
    if(x.IsEmpty())
      Begin = End = 0;
    else
  ...
  sINLINE T& GetTail()
  {
    return *GetType(GetNode(End)->Prev);
  }

  sINLINE sBool operator == (const sDListRange& x)
  {
    return Begin == x.Begin && End == x.End;
  }
  ...
  sINLINE void RemHead()
  {
    Begin = GetType(GetNode(Begin)->Next);
  }

  sINLINE void RemTail()
  {
    End = GetType(GetNode(End)->Prev);
  }
};

template<typename Type, sDNode Type::* Offset>
sINLINE sDListRange<Type, Offset> sAll(sDList<Type, Offset>& x)
{
  return sDListRange<Type, Offset>(x);
}
...
public:
  ...
  sINLINE ValueType& GetHead()
  {
    return Host.GetTail();
  }

  sINLINE ValueType& GetTail()
  {
    return Host.GetHead();
  }

  sINLINE sBool operator == (const ReverseRange<Range, RandomAccess>& x)
  {
    return Host == x.Host;
  }
...
public:
  ...
  sINLINE sInt GetCount() const
  {
    return ReverseRange<Range, sFALSE>::Host.GetCount();
  }

  sINLINE ValueType & operator [] (sInt pos) const
  {
    return ReverseRange<Range, sFALSE>::Host[ReverseRange < Range, sFALSE > ::Host.GetCount() - 1 - pos];
  }

  sINLINE ThisType Slice(sInt first, sInt last) const
  {
    sInt cnt = ReverseRange<Range, sFALSE>::Host.GetCount();
    return ReverseRange(ReverseRange<Range, sFALSE>::Host.Slice(cnt - 1 - last, cnt - 1 - first));
  }
...
template<typename T>
sINLINE AutoAny<T> WrapAny(const T& x)
{
  return x;
}
...
template<typename T>
WrappedType<T> WrapType(const T& t)
{
...
struct sCmpLess
{
  sINLINE sBool operator () (const T& a, const T& b) const
  {
    return a < b;
  }
};
...
struct MemberLess
{
  ...
  sINLINE sBool operator () (const T& a, const KeyType& b) const
  {
    return a.*Key < b;
  }

  sINLINE sBool operator () (const KeyType& a, const T& b) const
  {
    return a < b.*Key;
  }

  sINLINE sBool operator () (const KeyType& a, const KeyType& b) const
  {
    return a < b;
  }
};
...
struct MemberPtrLess
{
  ...
  sINLINE sBool operator () (const KeyType& a, const KeyType& b)
  {
    return a < b;
  }
};
...
template<typename T, typename KeyType>
sAlgoImpl::MemberLess<T, KeyType> sMemberLess(KeyType T::* key)
{
  return sAlgoImpl::MemberLess<T, KeyType>(key);
}

template<typename T, typename KeyType>
sAlgoImpl::MemberGreater<T, KeyType> sMemberGreater(KeyType T::* key)
{
  return sAlgoImpl::MemberGreater<T, KeyType>(key);
}

// Less and greater of a designated member (key) in when the comparands are pointers
template<typename T, typename KeyType>
sAlgoImpl::MemberPtrLess<T, KeyType> sMemPtrLess(KeyType T::* key)
{
  return sAlgoImpl::MemberPtrLess<T, KeyType>(key);
}

template<typename T, typename KeyType>
sAlgoImpl::MemberPtrGreater<T, KeyType> sMemPtrGreater(KeyType T::* key)
{
  return sAlgoImpl::MemberPtrGreater<T, KeyType>(key);
}
...
public:
  ...
  void ChangeStore(const Range& range)
  {
    Store = range;
  }
    ...
    void IntroSortR(const Range& range, sInt cutoff, const Pred& less)
    {
      ...
      if(count > ISortThreshold)
      {
        if(cutoff > 0) // use quicksort
        {
          ...
          for(;;)
          {
            while(less(r[i], pivot))
              i++;

            do
              --j;
            while(less(pivot, r[j]));
          ...
          while(j >= 0 && !less(r[j], pivot))
            j--;

          while(i < count && !less(pivot, r[i]))
            i++;
          ...
          if((n = j + 1) > 1)
            IntroSortR(range.Slice(0, n), cutoff, less);

          if((n = count - i) > 1)
            IntroSortR(range.Slice(i, count), cutoff, less);
        }
    ...
    void IntroSortR(const Range& range, sInt cutoff)
    {
      ...
      if(count > ISortThreshold)
      {
        if(cutoff > 0) // use quicksort
        {
          ...
          if((n = j + 1) > 1)
            IntroSortR(range.Slice(0, n), cutoff);

          if((n = count - i) > 1)
            IntroSortR(range.Slice(i, count), cutoff);
        }
        else // using too many recursions, try and finish it off with heapsort instead
          sHeapSort(range);
      }
