...
struct ThreadEvent
{
  ...
  bool operator == (const ThreadEvent& b) const
  {
    return !sCmpMem(this, &b, sizeof(ThreadEvent));
  }
};
...
// sU64 rxs=r.SizeX();
    ...
    for(sInt i = 0; i < t->NumEvents - 1; i++)
    {
      ...
      if(e1.Ev.Color && (e1.Ev.Name || e1.Ev.Name8))
      {
        ...
        if(!i || e1.Level >= t->Events[i - 1].Level)
          LineCount[line]++;
