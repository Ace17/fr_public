...
sBool sScanner::ScanRaw(sTextBuffer& tb, sInt opentoken, sInt closetoken)
{
  ...
  {
    sString<64> msg;
    sSPrintF(msg, L"scan raw: %d %s token(s) missing, starting at line %d", count, (count > 0) ? L"close" : L"open", startLine);
    Error(msg);
  }
...
sBool sScanner::ScanLine(sTextBuffer& tb)
{
  ...
  sString<64> msg;
  sSPrintF(msg, L"scan line: premature end of file reached");
  Error(msg);
...
void Unexpected(sScanner* scan)
{
  switch(scan->Token)
  {
  case sTOK_NAME:   scan->Error(L"unexpected identifier \"%s\"", scan->Name);
    break;
...
void sRegex::RemoveEmptyState()
{
  ...
  sFORALL(Machine, s)              // make sure empty states are really empty
  {
    if(s->Temp == 0)
    {
      if(s->Input.GetCount() > 0)
        sFatal(L"sRegex::RemoveEmptyState() state not empty (1)");

      if(s->Output.GetCount() > 0)
        sFatal(L"sRegex::RemoveEmptyState() state not empty (1)");
    }
  }
