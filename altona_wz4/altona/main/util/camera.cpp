...
void sFreeCamera::OnFrame(sInt delta, sViewport& vp)
{
  ...
  if(MouseControl)
  {
    ...
    if(Scheme.Pressed(INPUT_MBL))
      Moving += 1;

    if(Scheme.Pressed(INPUT_MBR))
      Moving -= 1;
  }
  ...
  if(KeyMask & 0x01)
    SpeedV = SpeedV - vp.Camera.i * (delta * Speed);

  if(KeyMask & 0x02)
    SpeedV = SpeedV + vp.Camera.i * (delta * Speed);

  if(KeyMask & 0x04)
    SpeedV = SpeedV + vp.Camera.k * (delta * Speed);
...
void sFreeCamera::OnKey(sU32 key)
{
  ...
  case 'a':             KeyMask |= 0x01;
    break;
  case 'a' | sKEYQ_BREAK: KeyMask &= ~0x01;
    break;
  case 'd':             KeyMask |= 0x02;
    break;
  case 'd' | sKEYQ_BREAK: KeyMask &= ~0x02;
    break;
  case 'w':             KeyMask |= 0x04;
    break;
  case 'w' | sKEYQ_BREAK: KeyMask &= ~0x04;
    break;
  case 's':             KeyMask |= 0x08;
    break;
  case 's' | sKEYQ_BREAK: KeyMask &= ~0x08;
    break;
  case 'q':             KeyMask |= 0x10;
    break;
  case 'q' | sKEYQ_BREAK: KeyMask &= ~0x10;
    break;
  case 'e':             KeyMask |= 0x20;
    break;
  case 'e' | sKEYQ_BREAK: KeyMask &= ~0x20;
    break;
...
void sCameraXSI::OnKey(sU32 key)
{
  switch(key)
  {
  case 'p':             KeyMaskCurrent |= XSI_DOLLY;
    break;
  case 'p' | sKEYQ_BREAK: KeyMaskCurrent &= ~XSI_DOLLY;
    break;
  case 'o':             KeyMaskCurrent |= XSI_ORBIT;
    break;
  case 'o' | sKEYQ_BREAK: KeyMaskCurrent &= ~XSI_ORBIT;
    break;
  case 'z':             KeyMaskCurrent |= XSI_TRANS_CAMPLANE;
    break;
  case 'z' | sKEYQ_BREAK: KeyMaskCurrent &= ~XSI_TRANS_CAMPLANE;
    break;
  case 'h':             Init();
    break;
  case 's':             KeyMaskCurrent |= XSI_SELECT;
    break;
  case 's' | sKEYQ_BREAK: KeyMaskCurrent &= ~XSI_SELECT;
    break;
    break;
  default:;
  }
