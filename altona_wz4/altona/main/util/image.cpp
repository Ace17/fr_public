...
static sDecompressImageDataHandler DecompressImageHandler[sICT_COUNT] =
{
  0
};
...
void sImageData::Serialize_(streamer& stream)
{
...
void sImageData::Serialize(sWriter& s)
{
  Serialize_(s);
}
  ...
  case sTEX_MRGB8:
    {
      ...
      for(sInt m = 1; m < img->Mipmaps; m++)
      {
        ...
        for(sInt y = 0; y < ysn; y++)
        {
          ...
          {
            sVector4 c0;
            c0.InitMRGB8(src[x * 2 + y * 2 * xs]);
            sVector4 c1;
            c0.InitMRGB8(src[x * 2 + 1 + y * 2 * xs]);
            sVector4 c2;
            c0.InitMRGB8(src[x * 2 + 1 + y * 2 * xs + xs]);
            sVector4 c3;
            c0.InitMRGB8(src[x * 2 + y * 2 * xs + xs]);
  ...
  case sTEX_MRGB16:
    {
      ...
      for(sInt m = 1; m < img->Mipmaps; m++)
      {
        ...
        for(sInt y = 0; y < ysn; y++)
        {
          ...
          {
            sVector4 c0;
            c0.InitMRGB16(src[x * 2 + y * 2 * xs]);
            sVector4 c1;
            c0.InitMRGB16(src[x * 2 + 1 + y * 2 * xs]);
            sVector4 c2;
            c0.InitMRGB16(src[x * 2 + 1 + y * 2 * xs + xs]);
            sVector4 c3;
            c0.InitMRGB16(src[x * 2 + y * 2 * xs + xs]);
            sVector4 result = (c0 + c1 + c2 + c3) * 0.25f;
...
template<class streamer>
void sImage::Serialize_(streamer& stream)
{
...
void sImage::Serialize(sWriter& stream)
{
  Serialize_(stream);
}

void sImage::Serialize(sReader& stream)
{
  Serialize_(stream);
}
...
sImage* sImage::Half(sBool gammacorrect) const
{
  ...
  if(gammacorrect)
    sVERIFYFALSE; // not yet implemented!
...
sBool sImage::Load(const sChar* name)
{
  ...
  if(sCmpStringI(ext, L"pic") == 0)
    result = LoadPIC(data, size);

  if(sCmpStringI(ext, L"tga") == 0)
    result = LoadTGA(name);

  if(sCmpStringI(ext, L"bmp") == 0)
    result = LoadBMP(data, size);

  if(sCmpStringI(ext, L"jpg") == 0)
    result = LoadJPG(data, size);

  if(sCmpStringI(ext, L"png") == 0)
    result = LoadPNG(data, size);
...
sBool sImage::Save(const sChar* name)
{
  ...
  if(!sCmpStringI(ext, L"bmp"))
    return SaveBMP(name);

  if(!sCmpStringI(ext, L"pic"))
    return SavePIC(name);

  if(!sCmpStringI(ext, L"tga"))
    return SaveTGA(name);

  if(!sCmpStringI(ext, L"png"))
    return SavePNG(name);
...
sBool sImage::LoadBMP(const sU8* data, sInt size)
{
  ...
  case 8:
    ...
    if(size < sInt(sizeof(BMPHeader)))
      return 0;  // space for palette
...
sBool sImage::LoadPIC(const sU8* data, sInt size)
{
  ...
  if(*data++ != 8)
    ok = sFALSE;           // bits per channel

  if(*data++ != 2)
    compressed = sFALSE;    // compressed/uncompressed

  if(*data++ != 0xe0)
    ok = sFALSE;        // channelcode
...
sBool sImage::SaveTGA(const sChar* name)
{
  ...
  tgaHead.colorMapOrigin[0] = 0;
  tgaHead.colorMapOrigin[1] = 0; // no colormap
  tgaHead.colorMapLength[0] = 0;
  tgaHead.colorMapLength[1] = 0; // no colormap
  tgaHead.colorMapEntrySize = 0; // no colormap
  tgaHead.xOrigin[0] = 0;
  tgaHead.xOrigin[1] = 0; // left
  tgaHead.yOrigin[0] = 0;
  tgaHead.yOrigin[1] = 0; // down
...
class sTextureCube* sLoadTextureCube(const sChar* t0, const sChar* t1, const sChar* t2, const sChar* t3, const sChar* t4, const sChar* t5, sInt formatandflags)
{
  ...
  if(formatandflags & sTEX_NOMIPMAPS)
    mipmaps = 1;
...
static inline sInt GetRed(sU32 col)
{
  return (col >> 16) & 0xff;
}

static inline sInt Pack8(sF32 val)
{
  return sInt((val + 1.0f) * 127.5f) & 0xff;
}
...
void sClampToARGB8(sImageData* img, sInt mm /*=0*/)
{
  ...
  while((img->SizeX >> check) >= 1 && (img->SizeY >> check) >= 1)
    check++;
...
void sImage::MonoToAll()
{
  ...
  {
    if((Data[i] & 0x00ffffff) != refc)
      incolor = sTRUE;

    if((Data[i] & 0xff000000) != refa)
      inalpha = sTRUE;
  }
  ...
  {
    if(incolor)
      sLogF(L"gfx", L"MonoToAll: texture has content in both channels, using alpha\n");
...
template<class streamer>
void sImageI16::Serialize_(streamer& s)
{
...
void sImageI16::Serialize(sWriter& s)
{
  Serialize_(s);
}

void sImageI16::Serialize(sReader& s)
{
  Serialize_(s);
}
