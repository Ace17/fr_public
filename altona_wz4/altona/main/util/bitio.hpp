...
public:
  ...
  sBool IsOk()
  {
    return !Error && (ExtraBytes < 4 || BitsLeft == 32);
  }
  ...
  sINLINE sU32 PeekBits(sInt count)
  {
    return BitBuffer >> (32 - count);
  }

  sINLINE sS32 PeekBitsS(sInt count)
  {
    return sS32(BitBuffer) >> (32 - count);
  }

  sINLINE sU32 GetBits(sInt count)
  {
    sU32 r = PeekBits(count);
    SkipBits(count);
    return r;
  }

  sINLINE sS32 GetBitsS(sInt count)
  {
    sS32 r = PeekBitsS(count);
    SkipBits(count);
    return r;
  }
};
