...
// void Init(sInt format,sInt xs,sInt ys,sInt mipmaps=1);    // obsolete
  ...
  sInt GetByteSize() const
  {
    return UnpackedSize;
  }

  sInt GetDiskSize() const
  {
    return DataSize;
  }

  sInt GetFaceSize() const
  {
    return CubeFaceSize;
  }
...
// void SetChannel(IChannel channelX,sU8 val);
  ...
  void HalfTransparentRect(const sRect& r, sU32 color)
  {
    HalfTransparentRect(r.x0, r.y0, r.x1, r.y1, color);
  }
...
// HDR compression
inline sBool sCheckMRGB(sInt format)
{
  format &= sTEX_FORMAT;
  return format == sTEX_MRGB8 || format == sTEX_MRGB16;
}
