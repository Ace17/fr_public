...
public:
  sScannerSource();
  virtual ~sScannerSource()
  {
  }    // destructor has to be virtual

  virtual sBool Refill()
  {
    return 1;
  }  // call this every linefeed (at least)
...
public:
  struct ScannerToken
  {
    ...
    sInt Length;
    sPoolString Name;
    sInt operator == (const ScannerToken& tok)
    {
  ...
  void SetFilename(const sChar* f)
  {
    Stream->Filename = f;
  }
  ...
  sPRINTING0(Error, sFormatStringBuffer buf = sFormatStringBase(TempString, format);
             buf, Error0(TempString);
             )
  sInt Warn0(const sChar* error);      // always returns 0, so you can do   return Scan.Error("bla");
  sPRINTING0(Warn, sFormatStringBuffer buf = sFormatStringBase(TempString, format);
             buf, Warn0(TempString);
             )
  ...
  const sChar* GetScanPosition()
  {
    return Stream->ScanPtr;
  }
...
public:
  ...
  sInt GetMatchCount()
  {
    return Valid ? Success.GetCount() : 0;
  }
