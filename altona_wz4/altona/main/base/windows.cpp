...
void sRender2DBegin(sInt xs, sInt ys)
{
  ...
  if(sGDIDC == 0)
    sFatal(L"could not create GDI-DC for bitmap rendering");

  Render2DBM = CreateCompatibleBitmap(screendc, xs, ys);

  if(Render2DBM == 0)
    sFatal(L"could not create GDI-BM for bitmap rendering");
