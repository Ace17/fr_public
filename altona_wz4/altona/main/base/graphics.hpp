...
sINLINE sBool sRender3DLockOwner()
{
  return sTRUE;
}
...
{
  sInt x, y;
  void Init(sInt x_, sInt y_)
  {
    x = x_;
    y = y_;
  }

  sBool operator == (const sScreenInfoXY& a) const
  {
    return x == a.x && y == a.y;
  }
};
...
sINLINE sU16 sFloatToHalf(sF32 f)
{
  ...
  return sU16(((i & 0x80000000) >> 16) | (((i & 0x7f800000) >> 13) - (127 << 10) + (15 << 10)) | ((i & 0x007fe000) >> 13));
}
...
struct sHalfInt   // in case we are using normalized 16 bit integers instead of floats...
{
  ...
  void Set(sF32 f)
  {
    Val = sU16(sInt((f + 1) * 32767.5f) - 0x8000) & 0xffff;
  }
};
...
public:
  sInt sOBSOLETE GetSize() const
  {
    return VertexSize[0];
  }          // size in bytes

  sInt GetSize(sInt stream) const
  {
  ...
  sInt GetStreams() const
  {
    return Streams;
  }                       // highest used stream + 1. if streams 0 and 2 are used, this is 3
...
struct sVertexBasic               // 16 bytes
{
  ...
  static sVertexFormatHandle* VertexFormat()
  {
    return sVertexFormatBasic;
  }
};
...
struct sVertexTSpace4_uv3         // 68 bytes, this will change a lot....
{
  ...
  static sVertexFormatHandle* VertexFormat()
  {
    return sVertexFormatTSpace4_uv3;
  }
};
...
struct sVertexInstancePlus
{
  ...
  static sVertexFormatHandle* VertexFormat()
  {
    return sVertexFormatInstancePlus;
  }
};
...
template<sInt Start, sInt Count>
struct CBMask
{
  template<sInt val, sInt MIN, sInt MAX>
  struct Clamp
  {
    enum
    {
...
public:
  ...
  sINLINE void Init(Type* data)
  {
    SetCfg(Type::Slot, Type::RegStart, Type::RegCount, CBMask<Type::RegStart, Type::RegCount>::Mask);
    SetPtr(0, data);
  }
...
template<class Type>
// you are supposed to create this as member variable or directly on stack
...
sINLINE void sSetCBuffers(sCBufferBase* cbuffers)
{
  sSetCBuffers(&cbuffers, 1);
}
...
public:
  ...
  void PMatrix(const sMatrix44& mat)
  {
    Proj = mat;
  }

  const sMatrix44& PMatrix() const
  {
    return Proj;
  }

  const sMatrix34& VMatrix() const
  {
    return View;
  }

  const sMatrix34& MVMatrix() const
  {
    return ModelView;
  }

  const sMatrix44& MVPMatrix() const
  {
    return ModelScreen;
  }
...
// sF32 Zoom,Aspect;
  ...
  {
    return ZoomX / ZoomY;
  }  // aspect is X/Y, like 16/9. Previously it was defined the other way around!
  ...
  {
    return Visible(box, ClipNear, ClipFar);
  }    // 0=total out, 1=clip, 2=total in
...
public:
  ...
  sINLINE void SetMorphTargetId(sU32 id)
  {
    MorphTargetId = id;
  };
  sINLINE sU32 GetMorphTargetId() const
  {
  ...
  template<typename T>
  void BeginLoadIB(sInt ic, sGeometryDuration duration, T** ip)
  {
    void** ptr = (void**)ip;
    BeginLoadIB(ic, duration, ptr);
  };
  template<typename T>
  void BeginLoadVB(sInt vc, sGeometryDuration duration, T** vp, sInt stream = 0)
  {
    void** ptr = (void**)vp;
    BeginLoadVB(vc, duration, ptr, stream);
  };
  template<typename V, typename I>
  void BeginLoad(sVertexFormatHandle* vh, sInt flags, sGeometryDuration duration, sInt vc, sInt ic, V** vp, I** ip)
  {
    void** vptr = (void**)vp;
    void** iptr = (void**)ip;
    BeginLoad(vh, flags, duration, vc, ic, vptr, iptr);
  };
  ...
  template<typename T>
  void BeginQuad(T** data, sInt count)
  {
    void** ptr = (void**)data;
    BeginQuad(ptr, count);
  };
  template<typename T>
  void BeginGrid(T** data, sInt xs, sInt ys)
  {
    void** ptr = (void**)data;
    BeginGrid(ptr, xs, ys);
  };
  template<typename T>
  void BeginWireGrid(T** data, sInt xs, sInt ys)
  {
    void** ptr = (void**)data;
    BeginWireGrid(ptr, xs, ys);
  };
  void EndQuad();
  void EndGrid();
  void EndWireGrid();
...
public:
  ...
  // casting
  sTexture2D* CastTex2D()
  {
    if((Flags & (sTEX_TYPE_MASK | sTEX_SOFTWARE)) == sTEX_2D)
      return (sTexture2D*)this;

    return 0;
  }

  sTextureCube* CastTexCube()
  {
    if((Flags & (sTEX_TYPE_MASK | sTEX_SOFTWARE)) == sTEX_CUBE)
      return (sTextureCube*)this;

    return 0;
  }

  sTexture3D* CastTex3D()
  {
    if((Flags & (sTEX_TYPE_MASK | sTEX_SOFTWARE)) == sTEX_3D)
      return (sTexture3D*)this;
...
sINLINE sTextureBase* sReadTexture(sReader& s)
{
  sTextureBase* tex = 0;
  sReadTexture(s, tex);
  return tex;
...
public:
  ...
  void sSOON_OBSOLETE GetSize(sInt& xs, sInt& ys, sInt& zs)
  {
    xs = SizeX;
    ys = SizeY;
    zs = 1;
...
class sTextureCube : public sTextureBase
{
private:
  friend void sCopyCubeFace(sTexture2D*, sTextureCube*, sTexCubeFace);
...
class sTexture3D : public sTextureBase
{
private:
#if sRENDERER == sRENDER_DX9
  virtual void OnLostDevice(sBool reinit = sFALSE)
...
public:
  sVertexFormatHandle* GetPreparedFormat() const
  {
...
public:
  ...
  void Set(sInt variant = 0)
  {
    Set(0, 0, variant);
  }

  void Set(sCBufferBase** cbuffers, sInt cbcount, sInt variant = 0);
  void Set(sCBufferBase* a)
  {
    Set(&a, 1);
  }

  void Set(sCBufferBase* a, sCBufferBase* b)
  {
    sCBufferBase* A[2] =
    {
      a, b
  ...
  void Set(sCBufferBase* a, sCBufferBase* b, sCBufferBase* c)
  {
    sCBufferBase* A[3] =
    {
      a, b, c
  ...
  void Set(sCBufferBase* a, sCBufferBase* b, sCBufferBase* c, sCBufferBase* d)
  {
    sCBufferBase* A[4] =
    {
      a, b, c, d
  ...
  void SetV(sCBufferBase* a, sInt variant = 0)
  {
    Set(&a, 1, variant);
  }

  void SetV(sCBufferBase* a, sCBufferBase* b, sInt variant = 0)
  {
    sCBufferBase* A[2] =
    {
      a, b
  ...
  void SetV(sCBufferBase* a, sCBufferBase* b, sCBufferBase* c, sInt variant = 0)
  {
    sCBufferBase* A[3] =
    {
      a, b, c
  ...
  void SetV(sCBufferBase* a, sCBufferBase* b, sCBufferBase* c, sCBufferBase* d, sInt variant = 0)
  {
    sCBufferBase* A[4] =
    {
      a, b, c, d
