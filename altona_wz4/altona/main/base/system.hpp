...
sInt sGetRunlevel();
#define sADDSUBSYSTEM(n, p, i, e) namespace sSubSystems { struct n ## _type { n ## _type() { sAddSubsystem(sTXT(# n), p, i, e); } \
                                                          } n ## _global; }
...
struct sDateAndTime
{
  ...
  bool operator == (const sDateAndTime& x) const
  {
    return Compare(x) == 0;
  }
...
sPRINTING1(sSysLogF, sFormatStringBuffer buf;
           sFormatStringBaseCtx(buf, format);
           buf, sSysLog(arg1, buf.Get());
           , const sChar*)
...
struct sTimeHelper
{
  ...
  const sChar* Msg;
  sU32* TotalTime;
  sTimeHelper(const sChar* msg) : Msg(msg)
  {
    TotalTime = 0;
    Start = sGetTime();
  }

  sTimeHelper(const sChar* msg, sU32& totaltime) : Msg(msg)
  {
    TotalTime = &totaltime;
    Start = sGetTime();
  }
...
sINLINE sU32 sGetDemoTimeOut()
{
  return 0;
}   // 0 : never, otherwise timeout time in ms
...
struct sThreadContext
{
  ...
  void* GetTls(sPtr offset)
  {
    if(offset < TlsOffset)
      return (void*)(((sU8*)this) + offset);
    else
      return 0;
  }
...
public:
  ...
  sBool CheckTerminate()
  {
    return !TerminateFlag;
  }

  sThreadContext* GetContext()
  {
    return Context;
  }
...
inline void* sGetTls(sPtr offset)
{
  return sGetThreadContext()->GetTls(offset);
}

template<typename T>
sPtr sAllocTls(T*& ptr, sInt align = 4)
{
  sPtr offset = sAllocTls(sizeof(T), align);
  ptr = sGetTls(offset);
  return offset;
}

template<typename T>
inline T* sGetTls(sPtr offset)
{
  return (T*)sGetThreadContext()->GetTls(offset);
}
...
inline sU32 sAtomicAdd(volatile sU32* p, sInt i)
{
  sU32 e = _InterlockedExchangeAdd((volatile long*)p, i);
  return e + i;
}

inline sU32 sAtomicInc(volatile sU32* p)
{
  return _InterlockedIncrement((long*)p);
}

inline sU32 sAtomicDec(volatile sU32* p)
{
  return _InterlockedDecrement((long*)p);
}

inline sU64 sAtomicAdd(volatile sU64* p, sInt i)
{
  sU64 e = _InterlockedExchangeAdd64((volatile __int64*)p, i);
  return e + i;
}

inline sU64 sAtomicInc(volatile sU64* p)
{
  return _InterlockedIncrement64((__int64*)p);
}

inline sU64 sAtomicDec(volatile sU64* p)
{
  return _InterlockedDecrement64((__int64*)p);
}

inline sU32 sAtomicSwap(volatile sU32* p, sU32 i)
{
  return _InterlockedExchange((long*)p, i);
}
...
inline sU32 sAtomicAdd(volatile sU32* p, sU32 i)
{
...
inline sU64 sAtomicAdd(volatile sU64* p, sU64 i)
{
  return __sync_add_and_fetch(p, i);
}
...
inline sU32 sAtomicSwap(volatile sU32* p, sU32 val)
{
...
inline sU32 sAtomicAdd(volatile sU32* p, sU32 i)
{
  return __builtin_cellAtomicAdd32((sU32*)p, i) + i;
}

inline sU32 sAtomicInc(volatile sU32* p)
{
  return __builtin_cellAtomicAdd32((sU32*)p, 1) + 1;
}

inline sU32 sAtomicDec(volatile sU32* p)
{
  return __builtin_cellAtomicAdd32((sU32*)p, ~0u) - 1;
}

inline sU64 sAtomicAdd(volatile sU64* p, sU64 i)
{
  return __builtin_cellAtomicAdd64((sU64*)p, i) + i;
}

inline sU64 sAtomicInc(volatile sU64* p)
{
  return __builtin_cellAtomicAdd64((sU64*)p, 1) + 1;
}

inline sU64 sAtomicDec(volatile sU64* p)
{
  return __builtin_cellAtomicAdd64((sU64*)p, ~0u) - 1;
}
...
static inline sU32 sAtomicSwap(volatile sU32* p, sU32 val)
{
  ...
  do
    prev = __builtin_cellAtomicLockLine32((sU32*)p);
  while(!__builtin_cellAtomicStoreConditional32((sU32*)p, val));
...
public:
  ...
  {
    return Host->Write(data, size);
  }

  virtual sU8* Map(sS64 offset, sDInt size)
  {
    return Host->Map(offset, size);
  }

  virtual sBool SetOffset(sS64 offset)
  {
    return Host->SetOffset(offset);
  }

  virtual sS64 GetOffset()
  {
    return Host->GetOffset();
  }

  virtual sBool SetSize(sS64 newSize)
  {
    return Host->SetSize(newSize);
  }
...
#else
sINLINE sInt sFileSeekTimeEmulation(sInt seektime_in_ms = -1)
{
  return 0;
}
