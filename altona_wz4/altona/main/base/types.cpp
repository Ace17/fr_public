...
{
  if(a[0] != 0 && a[1] == ':')
    return 1;      // does not handle "cdrom:"
...
void sMakeRelativePath(const sStringDesc& path, const sChar* relTo)
{
  ...
  {
    if(pcp >= 0)
      return; // we're lost. we don't know on what drive the rel path is.
  }
  else
  {
    if(pcp >= 0)
    {
      if(rcp != pcp || sCmpStringILen(p, relTo, pcp - 1))
        return; // different drive or protocol
  ...
  if(plen >= 2 && ((p[0] == '/' && p[1] == '/') || (p[0] == '\\' || p[1] == '\\')))
  {
    if(rlen >= 2 && ((relTo[0] == '/' && relTo[1] == '/') || (relTo[0] == '\\' || relTo[1] == '\\')))
    {
      ...
      return; // we'll have to give up here.
  }

  // now simply cut off the common parts
  while(*p && *relTo)
  {
    sInt pc = sLowerChar(*p);

    if(pc == '\\')
      pc = '/';

    sInt rc = sLowerChar(*relTo);
...
sBool sMatchWildcard(const sChar* wild, const sChar* text, sBool casesensitive, sBool pathsensitive)
{
  for(;;)
  {
    if(*wild == '*')
    {
      ...
      while(*text)
      {
        if(sMatchWildcard(wild, text, casesensitive, pathsensitive))
          return 1;
...
sBool sFormatString(const sStringDesc& desc, const sChar* s, const sChar** fp)
{
  ...
  while(c)
  {
    if(c == '%')
    {
      ...
      {
        string = (sChar*)(sDInt)sVARARG(fp, arg);
        arg++;
...
void sFormatStringBuffer::PrintFloat(const sFormatStringInfo& info, sF64 v)
{
  ...
  {
    if(frac == -1)
      frac = sMax(0, field - 2 - fi.Exponent - sign);
  }
...
sF32 sTiming::GetAverageDelta() const
{
  ...
  if(avg < 0.001f)
    avg = 0.001f; // at least 1 microsec, to avoid division by zero!
  ...
  if(sCheckLogFilter(module))
  {
    while(*text)
    {
      ...
      if(sLogPrefixLength > 0)
      {
        ...
        while(i < sLogPrefixLength + 3)
          buf[i++] = ' ';   // this line can't overflow!
      }
...
void sSetAllocFailTest(sF32 prob)
{
  sMemFailProbability = prob;
}

sF32 sGetAllocFailTest()
{
  return sMemFailProbability;
}
...
struct sDbgMemRange
{
  ...
  sDbgMemRange(sPtr start, sPtr end)
  {
    Start = start;
    End = end;
  }
...
void* sAllocMem_(sPtr size, sInt align, sInt flags)
{
  ...
  if(!p)
  {
    ...
    else
    {
      ...
      if(sMemoryLeakCheck && sOutOfMemory != sAMF_DEBUG)
        sMemoryLeaks->DumpLeaks(L"", 0, sOutOfMemory);
...
void sMemMark(sBool fatal /*=sTRUE*/)
{
  ...
  if(sMemoryMark.Taken)
  {
    if(sMemoryMark.Hash != hash)
    {
      ...
      if(fatal)
        sFatal(L"memory leaks are fatal.\n");
    }
  }
  else  // or save state
  {
...
void sPartitionMemory(sPtr frame, sPtr dma, sPtr gfx)
{
  ...
  if(sMemDmaSize != dma && dma != 1)
  {
    sRender3DFlush();
    sMemDmaSize = 0;
    sFreeMem((void*)sMemDmaPtr[0]);
    sMemDmaPtr[0] = 0;
    sFreeMem((void*)sMemDmaPtr[1]);
    sMemDmaPtr[1] = 0;
  }
...
{
  if(ctx->FrameBorrow == 0)
    sFatal(L"sAllocFrameEnd without sAllocFrameBegin");
...
void* sAllocFrame(sPtr size, sInt align)
{
  ...
  if(ctx->FrameBorrow)
    sFatal(L"can't call sAllocFrame() between sAllocFrameBegin() and sAllocFrameEnd()");
...
void* sAllocFrameBegin(sInt size, sInt align)
{
  ...
  if(ctx->FrameBorrow)
    sFatal(L"can't call sAllocFrameBegin() between sAllocFrameBegin() and sAllocFrameEnd()");
...
void* sAllocDma(sPtr size, sInt align)
{
  ...
  if(ctx->DmaBorrow)
    sFatal(L"can't call sAllocDma() between sAllocDmaBegin() and sAllocDmaEnd()");
...
void* sAllocDmaBegin(sInt size, sInt align /*=16*/)
{
  ...
  if(ctx->DmaBorrow)
    sFatal(L"can't call sAllocDmaBegin() between sAllocDmaBegin() and sAllocDmaEnd()");
...
void sAllocDmaEnd(void* ptr)
{
  ...
  if(ctx->DmaBorrow == 0)
    sFatal(L"sAllocDmaEnd without sAllocDmaBegin");
...
void sFlipMem()
{
  ...
  if(sFrameMemDoubleBuffered)
    sMemFrameToggle = !sMemFrameToggle;
...
void sPushMemLeakDesc(const sChar* desc)
{
  ...
  if(tc->MLDBPos > 0)
    tc->MemLeakDescBuffer2[tc->MLDBPos - 1] = ' ';
...
void sMemoryLeakTracker2::DumpLeaks(const sChar* message, sInt minallocid, sInt heapid)
{
  ...
  if(locs.GetCount() > 0)
  {
    ...
    if(!heapid || (info->HeapId & sAMF_MASK) == (heapid & sAMF_MASK))
    {
...
static sBool cmp(sMemoryLeakTracker2::LeakLoc* loc, sInt line, sInt heapid, const char* file)
{
  ...
  if(loc->HeapId != heapid)
    return 0;

  if(loc->File == file)
    return 1;

  if(loc->File == 0 || file == 0)
    return 0;

  for(sInt i = 0; loc->File[i] || file[i]; i++)
    if(loc->File[i] != file[i])
      return 0;
...
void sMemoryHeap::Init(sU8* start, sPtr size)
{
  ...
  if(Clear)
    sSetMem((void*)Start, 0xaa, End - Start);
...
void* sMemoryHeap::Alloc(sPtr bytes, sInt align, sInt flags)
{
  ...
  if(/*size<0 ||*/ size > TotalFree)
    return 0;       // no way we can do it
...
void sMoveMem(void* dd, const void* ss, sInt c)
{
  memmove(dd, ss, c);
}
...
{
  return fmod(over, under);
}

#endif

sInt sAbsMod(sInt over, sInt under)
{
  return (over >= 0) ? (over % under) : (under - (-over % under));
}

sF32 sAbsMod(sF32 over, sF32 under)
{
  return (over >= 0.0f) ? sMod(over, under) : (under - sMod(-over, under));
}
...
sF32 sRoundZero(sF32 f)
{
  return (f >= 0.0f) ? sFFloor(f) : sFCeil(f);
}

#endif

#ifndef sHASINTRINSIC_ROUNDNEAR
sF32 sRoundNear(sF32 f)
{
...
sInt sRoundDownInt(sF32 f)
{
  return sInt(floorf(f));
}

#endif

#ifndef sHASINTRINSIC_ROUNDUPI
sInt sRoundUpInt(sF32 f)
{
  return sInt(ceilf(f));
}
...
sInt sRoundNearInt(sF32 f)
{
  return sInt(floorf(f + 0.5f));
}
...
sF32 sSelectEQ(sF32 a, sF32 b, sF32 t, sF32 f)
{
  return (a == b) ? t : f;
}
...
sF32 sSelectNE(sF32 a, sF32 b, sF32 t, sF32 f)
{
  return (a != b) ? t : f;
}
...
sF32 sSelectGT(sF32 a, sF32 b, sF32 t, sF32 f)
{
  return (a >= b) ? t : f;
}
...
sF32 sSelectGE(sF32 a, sF32 b, sF32 t, sF32 f)
{
  return (a > b) ? t : f;
}
...
sF32 sSelectLT(sF32 a, sF32 b, sF32 t, sF32 f)
{
  return (a <= b) ? t : f;
}
...
sF32 sSelectLE(sF32 a, sF32 b, sF32 t, sF32 f)
{
  return (a < b) ? t : f;
}
...
sF32 sSelectEQ(sF32 a, sF32 b, sF32 t)
{
  return (a == b) ? t : 0;
}
...
sF32 sSelectNE(sF32 a, sF32 b, sF32 t)
{
  return (a != b) ? t : 0;
}
...
sF32 sSelectGT(sF32 a, sF32 b, sF32 t)
{
  return (a >= b) ? t : 0;
}
...
sF32 sSelectGE(sF32 a, sF32 b, sF32 t)
{
  return (a > b) ? t : 0;
}
...
sF32 sSelectLT(sF32 a, sF32 b, sF32 t)
{
  return (a <= b) ? t : 0;
}
...
sF32 sSelectLE(sF32 a, sF32 b, sF32 t)
{
  return (a < b) ? t : 0;
}
...
sF32 sLog2(sF32 f)
{
  return logf(f) / 0.69314718055994530941723212145818f;
}
...
void sSinCos(sF32 f, sF32& s, sF32& c)
{
  s = sinf(f);
  c = cosf(f);
}
...
sF32 sATan2(sF32 a, sF32 b)
{
  return (sF32)atan2(a, b);
}
...
void sFSinCos(sF32 f, sF32& s, sF32& c)
{
  s = sinf(f);
  c = cosf(f);
}
