...
sInt sDateAndTime::Compare(const sDateAndTime& x) const
{
  ...
  if((d = Minute - x.Minute) != 0)
    return d;

  if((d = Second - x.Second) != 0)
    return d;
...
sInt sFile::CopyFrom(sFile* f, sS64 max)
{
  ...
  while(max)
  {
    ...
    if(!f->Read(buffer, todo))
      break;

    if(!Write(buffer, todo))
      break;
