...
sBool sRay::HitPlane(sVector31& intersect, sVector4Arg plane) const
{
  ...
  if(dist < 0)
    return 0;            // already below plane.

  angle = -(plane ^ Dir);           // direction

  if(angle < sEPSILON)
    return 0;    // ray away from plane
...
void sSRT::Invert()
{
  ...
  Translate = mat.l;
  mat.Trans3();
  Scale.x = mat.i.Length();
  mat.i.Unit();
  Scale.y = mat.j.Length();
  mat.j.Unit();
  Scale.z = mat.k.Length();
  mat.k.Unit();
  mat.Trans3();
...
sF32 sSmooth(sF32 x, sF32 s)
{
  if(x <= 0)
    ...
    return (s * 0.5f) * (x * x) - (s - 4) * (x * x) * x + (s * 0.5f - 3) * (x * x) * (x * x);
}
...
void sPerlinDerive3D(sInt x, sInt y, sInt z, sInt mask, sInt seed, sF32& value, sVector30& dir)
{
  ...
  v110 = sPerlinPermute[vx0 + sPerlinPermute[vy1 + sPerlinPermute[vz1 ^ seed]]];
  v111 = sPerlinPermute[vx1 + sPerlinPermute[vy1 + sPerlinPermute[vz1 ^ seed]]];
  px000 = sPerlinRandom3D[v000][0];
  py000 = sPerlinRandom3D[v000][1];
  pz000 = sPerlinRandom3D[v000][2];
  px001 = sPerlinRandom3D[v001][0];
  py001 = sPerlinRandom3D[v001][1];
  pz001 = sPerlinRandom3D[v001][2];
  px010 = sPerlinRandom3D[v010][0];
  py010 = sPerlinRandom3D[v010][1];
  pz010 = sPerlinRandom3D[v010][2];
  px011 = sPerlinRandom3D[v011][0];
  py011 = sPerlinRandom3D[v011][1];
  pz011 = sPerlinRandom3D[v011][2];
  px100 = sPerlinRandom3D[v100][0];
  py100 = sPerlinRandom3D[v100][1];
  pz100 = sPerlinRandom3D[v100][2];
  px101 = sPerlinRandom3D[v101][0];
  py101 = sPerlinRandom3D[v101][1];
  pz101 = sPerlinRandom3D[v101][2];
  px110 = sPerlinRandom3D[v110][0];
  py110 = sPerlinRandom3D[v110][1];
  pz110 = sPerlinRandom3D[v110][2];
  px111 = sPerlinRandom3D[v111][0];
  py111 = sPerlinRandom3D[v111][1];
  pz111 = sPerlinRandom3D[v111][2];
