...
void PreInitGFX(sInt& flags, sInt& xs, sInt& ys)
{
  ...
  if(!(DXScreenMode.Flags & sSM_VALID))
  {
    ...
    if(flags & sISF_FULLSCREEN)
      DXScreenMode.Flags |= sSM_FULLSCREEN;

    if(flags & sISF_NOVSYNC)
      DXScreenMode.Flags |= sSM_NOVSYNC;

    if(flags & sISF_FSAA)
      DXScreenMode.MultiLevel = 255;

    if(flags & sISF_REFRAST)
      DXScreenMode.Flags |= sSM_REFRAST;
...
void InitGFX(sInt flags_, sInt xs_, sInt ys_)
{
  ...
  if(DXDev == 0)
  {
    ...
    if(DXScreenMode.Flags & sSM_REFRAST)
      dt = D3D_DRIVER_TYPE_REFERENCE;

    if(DXScreenMode.Flags & sSM_WARP)
...
// DXSwapChain[0]->AddRef();
  ...
  for(sInt i = 0; i < 64; i++)
  {
    if((1ULL << i) & avail)
    {
      ...
      s_res = 0;
      DXDev->CheckFormatSupport(fmt_res, &s_res);
      s_tex = 0;
      DXDev->CheckFormatSupport(fmt_tex, &s_tex);
      s_view = 0;
      DXDev->CheckFormatSupport(fmt_view, &s_view);
...
void sVertexFormatHandle::Create()
{
  ...
  for(sInt i = 0; Data[i]; i++)
  {
    ...
    {
    case sVF_NOP:   break;
    case sVF_POSITION:    decl[count].SemanticName = "POSITION";
      decl[count].SemanticIndex = 0;
      break;
    case sVF_NORMAL:      decl[count].SemanticName = "NORMAL";
      decl[count].SemanticIndex = 0;
      break;
    case sVF_TANGENT:     decl[count].SemanticName = "TANGENT";
      decl[count].SemanticIndex = 0;
      break;
    case sVF_BONEINDEX:   decl[count].SemanticName = "BLENDINDICES";
      decl[count].SemanticIndex = 0;
      break;
    case sVF_BONEWEIGHT:  decl[count].SemanticName = "BLENDWEIGHT";
      decl[count].SemanticIndex = 0;
      break;
    case sVF_BINORMAL:    decl[count].SemanticName = "BINORMAL";
      decl[count].SemanticIndex = 0;
      break;
    case sVF_COLOR0:      decl[count].SemanticName = "COLOR";
      decl[count].SemanticIndex = 0;
      break;
    case sVF_COLOR1:      decl[count].SemanticName = "COLOR";
      decl[count].SemanticIndex = 1;
      break;
    case sVF_COLOR2:      decl[count].SemanticName = "COLOR";
      decl[count].SemanticIndex = 2;
      break;
    case sVF_COLOR3:      decl[count].SemanticName = "COLOR";
      decl[count].SemanticIndex = 3;
      break;
    case sVF_UV0:         decl[count].SemanticName = "TEXCOORD";
      decl[count].SemanticIndex = 0;
      break;
    case sVF_UV1:         decl[count].SemanticName = "TEXCOORD";
      decl[count].SemanticIndex = 1;
      break;
    case sVF_UV2:         decl[count].SemanticName = "TEXCOORD";
      decl[count].SemanticIndex = 2;
      break;
    case sVF_UV3:         decl[count].SemanticName = "TEXCOORD";
      decl[count].SemanticIndex = 3;
      break;
    case sVF_UV4:         decl[count].SemanticName = "TEXCOORD";
      decl[count].SemanticIndex = 4;
      break;
    case sVF_UV5:         decl[count].SemanticName = "TEXCOORD";
      decl[count].SemanticIndex = 5;
      break;
    case sVF_UV6:         decl[count].SemanticName = "TEXCOORD";
      decl[count].SemanticIndex = 6;
      break;
    case sVF_UV7:         decl[count].SemanticName = "TEXCOORD";
      decl[count].SemanticIndex = 7;
      break;
    default: sVERIFYFALSE;
    }
    switch(Data[i] & sVF_TYPEMASK)
    {
    case sVF_F2:  decl[count].Format = DXGI_FORMAT_R32G32_FLOAT;
      b[stream] += 2 * 4;
      break;
    case sVF_F3:  decl[count].Format = DXGI_FORMAT_R32G32B32_FLOAT;
      b[stream] += 3 * 4;
      break;
    case sVF_F4:  decl[count].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
      b[stream] += 4 * 4;
      break;
    case sVF_I4:  decl[count].Format = DXGI_FORMAT_R8G8B8A8_UINT;
      b[stream] += 1 * 4;
      break;
    case sVF_C4:  decl[count].Format = DXGI_FORMAT_B8G8R8A8_UNORM;
      b[stream] += 1 * 4;
      break;
    case sVF_S2:  decl[count].Format = DXGI_FORMAT_R16G16_SNORM;
      b[stream] += 1 * 4;
      break;
    case sVF_S4:  decl[count].Format = DXGI_FORMAT_R16G16B16A16_SNORM;
      b[stream] += 2 * 4;
      break;
    case sVF_H2:  decl[count].Format = DXGI_FORMAT_R16G16_FLOAT;
      b[stream] += 1 * 4;
      break;
    case sVF_H4:  decl[count].Format = DXGI_FORMAT_R16G16B16A16_FLOAT;
      b[stream] += 2 * 4;
      break;
    case sVF_F1:  decl[count].Format = DXGI_FORMAT_R32_FLOAT;
      b[stream] += 1 * 4;
      break;
  ...
  for(sInt i = 0; Data[i]; i++)
  {
    ...
    switch(Data[i] & sVF_TYPEMASK)
    {
    case sVF_F2:  tb.Print(L"float2 ");
      break;
    case sVF_F3:  tb.Print(L"float3 ");
      break;
    case sVF_F4:  tb.Print(L"float4 ");
      break;
    case sVF_I4:  tb.Print(L"uint4 ");
      break;
    case sVF_C4:  tb.Print(L"float4 ");
      break;
    case sVF_S2:  tb.Print(L"float2 ");
      break;
    case sVF_S4:  tb.Print(L"float4 ");
      break;
    case sVF_H2:  tb.Print(L"float2 ");
      break;
    case sVF_H4:  tb.Print(L"float4 ");
      break;
    case sVF_F1:  tb.Print(L"float1 ");
      break;
    }
    ...
    switch(Data[i] & sVF_USEMASK)
    {
    case sVF_POSITION:    tb.Print(L"POSITION");
      break;
    case sVF_NORMAL:      tb.Print(L"NORMAL");
      break;
    case sVF_TANGENT:     tb.Print(L"TANGENT");
      break;
    case sVF_BONEINDEX:   tb.Print(L"BLENDINDICES");
      break;
    case sVF_BONEWEIGHT:  tb.Print(L"BLENDWEIGHT");
      break;
    case sVF_BINORMAL:    tb.Print(L"BINORMAL");
      break;
    case sVF_COLOR0:      tb.Print(L"COLOR0");
      break;
    case sVF_COLOR1:      tb.Print(L"COLOR1");
      break;
    case sVF_COLOR2:      tb.Print(L"COLOR2");
      break;
    case sVF_COLOR3:      tb.Print(L"COLOR3");
      break;
    case sVF_UV0:         tb.Print(L"TEXCOORD0");
      break;
    case sVF_UV1:         tb.Print(L"TEXCOORD1");
      break;
    case sVF_UV2:         tb.Print(L"TEXCOORD2");
      break;
    case sVF_UV3:         tb.Print(L"TEXCOORD3");
      break;
    case sVF_UV4:         tb.Print(L"TEXCOORD4");
      break;
    case sVF_UV5:         tb.Print(L"TEXCOORD5");
      break;
    case sVF_UV6:         tb.Print(L"TEXCOORD6");
      break;
    case sVF_UV7:         tb.Print(L"TEXCOORD7");
      break;
    }
...
void sGeometry::Init(sInt flags, sVertexFormatHandle* vf)
{
  ...
  {
  case sGF_TRILIST:     Topology = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
    break;
  case sGF_TRISTRIP:    Topology = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP;
    break;
  case sGF_LINELIST:    Topology = D3D11_PRIMITIVE_TOPOLOGY_LINELIST;
    break;
  case sGF_LINESTRIP:   Topology = D3D11_PRIMITIVE_TOPOLOGY_LINESTRIP;
    break;
  case sGF_LINELISTADJ: Topology = D3D11_PRIMITIVE_TOPOLOGY_LINELIST_ADJ;
    break;
  case sGF_TRILISTADJ:  Topology = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST_ADJ;
    break;
  case sGF_QUADLIST:    Topology = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
    break;
  case sGF_PATCHLIST:   Topology = D3D11_PRIMITIVE_TOPOLOGY_1_CONTROL_POINT_PATCHLIST
                                   - 1 + (flags & sGF_PATCHMASK);
    break;
  default: sVERIFYFALSE;
  }
}
...
void sGeometry::Draw(const sGeometryDrawInfo& di)
{
  ...
  else
  {
    ...
    switch(Flags & sGF_PRIMMASK)
    {
    case sGF_TRILIST:     Stats.Primitives += (ic / 3);
      break;
    case sGF_TRISTRIP:    Stats.Primitives += (ic - 2);
      break;
    case sGF_LINELIST:    Stats.Primitives += (ic / 2);
      break;
    case sGF_LINESTRIP:   Stats.Primitives += (ic - 1);
      break;
    case sGF_LINELISTADJ: Stats.Primitives += (ic / 4);
      break;
    case sGF_TRILISTADJ:  Stats.Primitives += (ic / 6);
      break;
    case sGF_PATCHLIST:   Stats.Primitives += ic / (Flags & sGF_PATCHMASK);
      break;
    }
...
void ConvertFlags(sU32 flags, sInt& mm, D3D11_BIND_FLAG& bind, D3D11_USAGE& usage, DXGI_FORMAT& fmt_res, DXGI_FORMAT& fmt_tex, DXGI_FORMAT& fmt_view, sBool& ds)
{
  ...
  {
  case sTEX_ARGB8888: fmt_res = DXGI_FORMAT_B8G8R8A8_UNORM;
    break;
  case sTEX_QWVU8888: fmt_res = DXGI_FORMAT_R8G8B8A8_SNORM;
    break;

  case sTEX_GR16:     fmt_res = DXGI_FORMAT_R16G16_UNORM;
    break;
  case sTEX_ARGB16:   fmt_res = DXGI_FORMAT_R16G16B16A16_UNORM;
    break;

  case sTEX_R32F:     fmt_res = DXGI_FORMAT_R32_FLOAT;
    break;
  case sTEX_GR32F:    fmt_res = DXGI_FORMAT_R32G32_FLOAT;
    break;
  case sTEX_ARGB32F:  fmt_res = DXGI_FORMAT_R32G32B32A32_FLOAT;
    break;
  case sTEX_R16F:     fmt_res = DXGI_FORMAT_R16_FLOAT;
    break;
  case sTEX_GR16F:    fmt_res = DXGI_FORMAT_R16G16_FLOAT;
    break;
  case sTEX_ARGB16F:  fmt_res = DXGI_FORMAT_R16G16B16A16_FLOAT;
    break;

  case sTEX_A8:       fmt_res = DXGI_FORMAT_A8_UNORM;
    break;
  case sTEX_I8:       fmt_res = DXGI_FORMAT_R8_UNORM;
    break;
  case sTEX_DXT1:     fmt_res = DXGI_FORMAT_BC1_UNORM;
    break;
  case sTEX_DXT1A:    fmt_res = DXGI_FORMAT_BC1_UNORM;
    break;
  case sTEX_DXT3:     fmt_res = DXGI_FORMAT_BC2_UNORM;
    break;
  case sTEX_DXT5:     fmt_res = DXGI_FORMAT_BC3_UNORM;
    break;
  case sTEX_DXT5N:    fmt_res = DXGI_FORMAT_BC3_UNORM;
    break;
  case sTEX_ARGB1555: fmt_res = DXGI_FORMAT_B5G5R5A1_UNORM;
    break;
  case sTEX_RGB565:   fmt_res = DXGI_FORMAT_B5G6R5_UNORM;
    break;

  case sTEX_MRGB8:    fmt_res = DXGI_FORMAT_B8G8R8A8_UNORM;
    break;
  case sTEX_8TOIA:    fmt_res = DXGI_FORMAT_R8_UNORM;
    break;
  ...
  case sTEX_STRUCTURED: fmt_res = DXGI_FORMAT_UNKNOWN;
    break;
  case sTEX_RAW:        fmt_res = DXGI_FORMAT_R32_UINT;
    break;
  case sTEX_UINT32:     fmt_res = DXGI_FORMAT_R32_UINT;
    break;
  ...
  if(fmt_tex == DXGI_FORMAT_UNKNOWN)
    fmt_tex = fmt_res;

  if(fmt_view == DXGI_FORMAT_UNKNOWN)
    fmt_view = fmt_res;
...
void sTextureProxy::Connect2()
{
  ...
  DXTex2D = Link->DXTex2D;

  if(DXTex2D)
    DXTex2D->AddRef();

  DXTexView = Link->DXTexView;

  if(DXTexView)
    DXTexView->AddRef();

  DXRenderView = Link->DXRenderView;

  if(DXRenderView)
    DXRenderView->AddRef();

  DXDepthView = Link->DXDepthView;
...
void sMaterial::Prepare(sVertexFormatHandle* format)
{
  ...
  VertexShader = 0;
  shaders[1] = HullShader;
  HullShader = 0;
  shaders[2] = DomainShader;
  DomainShader = 0;
  shaders[3] = GeometryShader;
  GeometryShader = 0;
  shaders[4] = PixelShader;
  PixelShader = 0;
  shaders[5] = ComputeShader;
  ComputeShader = 0;
...
void sMaterial::SetVariant(sInt var)
{
  ...
  if(!(Flags & sMTRL_MSK_RED))
    bs.RenderTarget[0].RenderTargetWriteMask |= D3D11_COLOR_WRITE_ENABLE_RED;

  if(!(Flags & sMTRL_MSK_GREEN))
    bs.RenderTarget[0].RenderTargetWriteMask |= D3D11_COLOR_WRITE_ENABLE_GREEN;

  if(!(Flags & sMTRL_MSK_BLUE))
    bs.RenderTarget[0].RenderTargetWriteMask |= D3D11_COLOR_WRITE_ENABLE_BLUE;

  if(!(Flags & sMTRL_MSK_ALPHA))
    bs.RenderTarget[0].RenderTargetWriteMask |= D3D11_COLOR_WRITE_ENABLE_ALPHA;
  // depth
  ...
  for(sInt i = 0; i < sMTRL_MAXTEX; i++)
  {
    if(Texture[i] || (TFlags[i] & sMTF_EXTERN))
    {
      ...
      {
      case sMTF_LEVEL0: sd.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
        break;
      case sMTF_LEVEL1: sd.Filter = D3D11_FILTER_MIN_MAG_LINEAR_MIP_POINT;
        break;
      case sMTF_LEVEL2: sd.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
        break;
      case sMTF_LEVEL3: sd.Filter = D3D11_FILTER_ANISOTROPIC;
        break;
      }
      ...
      {
      case sMTF_TILE_U:   sd.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;
        break;
      case sMTF_CLAMP_U:  sd.AddressU = D3D11_TEXTURE_ADDRESS_CLAMP;
        break;
      case sMTF_MIRROR_U: sd.AddressU = D3D11_TEXTURE_ADDRESS_MIRROR;
        break;
      case sMTF_BORDER_U: sd.AddressU = D3D11_TEXTURE_ADDRESS_BORDER;
        break;
      }
      switch(TFlags[i] & sMTF_ADDRMASK_V)
      {
      case sMTF_TILE_V:   sd.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
        break;
      case sMTF_CLAMP_V:  sd.AddressV = D3D11_TEXTURE_ADDRESS_CLAMP;
        break;
      case sMTF_MIRROR_V: sd.AddressV = D3D11_TEXTURE_ADDRESS_MIRROR;
        break;
      case sMTF_BORDER_V: sd.AddressV = D3D11_TEXTURE_ADDRESS_BORDER;
        break;
      }
      switch(TFlags[i] & sMTF_ADDRMASK_W)
      {
      case sMTF_TILE_W:   sd.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;
        break;
      case sMTF_CLAMP_W:  sd.AddressW = D3D11_TEXTURE_ADDRESS_CLAMP;
        break;
      case sMTF_MIRROR_W: sd.AddressW = D3D11_TEXTURE_ADDRESS_MIRROR;
        break;
      case sMTF_BORDER_W: sd.AddressW = D3D11_TEXTURE_ADDRESS_BORDER;
        break;
      }
