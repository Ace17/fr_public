...
sInt sRootFileHandler::AllocReadHandle()
{
  ...
  if(e < 0)
    sFatal(L"sRootFileHandler: out of async read entries!\n");
...
sFile* sRootFileHandler::Create(const sChar* name, sFileAccess access)
{
  ...
  static const sChar* mode[] =
  {
    0, L"read", L"readrandom", L"write", L"writeappend", L"readwrite"
  };
...
void sGetTempDir(const sStringDesc& str)
{
  ...
  while((pos = sFindFirstChar(str.Buffer, '\\')) >= 0)
    str.Buffer[pos] = '/';
}
...
static sBool sStackTraceFromContext(const sStringDesc& tgt, sInt skipCount, sInt maxCount, CONTEXT& ctx)
{
  ...
  if(!DbgHelpInited)
  {
    ...
    static const sChar* envs[] =
    {
      L"windir", L"_NT_SYMBOL_PATH", L"_NT_ALTERNATE_SYMBOL_PATH"
    };
...
sBool sGetMemTagged()
{
  return sGetThreadContext()->TagMemFile != 0;
}
...
/*          if (rm->usFlags & MOUSE_MOVE_ABSOLUTE)
    ...
    case WM_SYSKEYUP:
      ...
      if(wparam == VK_SHIFT && !i)
        sKeyQual &= ~sKEYQ_SHIFTL;

      if(wparam == VK_SHIFT && i)
        sKeyQual &= ~sKEYQ_SHIFTR;

      if(wparam == VK_CONTROL && !i)
        sKeyQual &= ~sKEYQ_CTRLL;

      if(wparam == VK_CONTROL && i)
        sKeyQual &= ~sKEYQ_CTRLR;

      if(wparam == VK_MENU && !i)
        sKeyQual &= ~sKEYQ_ALT;

      if(wparam == VK_MENU && i)
        sKeyQual &= ~sKEYQ_ALTGR;

      t = 0;

      if((sKeyQual & sKEYQ_SHIFT) || (sKeyQual & sKEYQ_CAPS))
        t = 1;

      if(sKeyQual & sKEYQ_ALTGR)
      {
        t = 2;
        sKeyQual &= ~sKEYQ_CTRLL;
      }
...
void sExternMainExit()
{
  ...
  if(sHWND)
  {
    ...
    if(sHWND)
      UnregisterClassW(L"EXTERN_ALTONA", WInstance);
  }
...
class sVSHeapBase : public sMemoryHandler
{
  ...
  {
    sCONFIG_SIZET total, largest;
    GetFreeStats(total, largest);
    return total;
  }
...
public:
  void* Alloc(sPtr size, sInt align, sInt flags)
  {
    ...
    tx; // "tx;" to fix unused local variable warning
...
{
  if(sDebugHeapBase)
    VirtualFree(sDebugHeapBase, 0, MEM_RELEASE);

  if(sMainHeapBase)
    VirtualFree(sMainHeapBase, 0, MEM_RELEASE);
