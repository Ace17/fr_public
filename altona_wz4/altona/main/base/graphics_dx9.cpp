...
static void MakeDX9()
{
  if(!DX9)
  {
    ...
    if(!DX9)
      sFatal(L"Could not create DirectX9");
  }
}
...
void GetScreenFormats(sInt display, sInt& flags, D3DFORMAT& color, D3DFORMAT& depth, D3DFORMAT& displayfmt)
{
  ...
  for(sInt check = 0; check < 5; check++)
  {
    ...
    {
    case 0: break;
    case 1: flags &= ~sSM_DEPTHREAD;
      break;
    case 2: flags &= ~sSM_STENCIL;
      break;
    }

    if(flags & sSM_DEPTH16BIT)
      id |= 1;

    if(flags & sSM_DEPTHREAD)
      id |= 2;

    if(flags & sSM_STENCIL)
      id |= 4;
...
void sClearRendertargetPrivate(sInt flags, sU32 color)
{
  ...
  if(!(DXScreenMode.Flags & sSM_STENCIL))
...
sTexture2D* sGetCurrentBackBuffer()
{
  return DXBackBuffer;
}

sTexture2D* sGetCurrentBackZBuffer()
{
  return DXZBuffer;
}
...
void sBeginSaveRTPrivate(const sU8*& data, sS32& pitch, enum sTextureFlags& flags, IDirect3DSurface9* source)
{
  ...
  case D3DFMT_A8R8G8B8:       flags = sTEX_ARGB8888;
    break;
  case D3DFMT_Q8W8V8U8:       flags = sTEX_QWVU8888;
    break;

  case D3DFMT_G16R16:         flags = sTEX_GR16;
    break;
  case D3DFMT_A16B16G16R16:   flags = sTEX_ARGB16;
    break;

  case D3DFMT_R32F:           flags = sTEX_R32F;
    break;
  case D3DFMT_G32R32F:        flags = sTEX_GR32F;
    break;
  case D3DFMT_A32B32G32R32F:  flags = sTEX_ARGB32F;
    break;
  case D3DFMT_R16F:           flags = sTEX_R16F;
    break;
  case D3DFMT_G16R16F:        flags = sTEX_GR16F;
    break;
  case D3DFMT_A16B16G16R16F:  flags = sTEX_ARGB16F;
    break;

  case D3DFMT_A8:             flags = sTEX_A8;
    break;
  case D3DFMT_L8:             flags = sTEX_I8;
    break;
  case D3DFMT_A1R5G5B5:       flags = sTEX_ARGB1555;
    break;
  case D3DFMT_A4R4G4B4:       flags = sTEX_ARGB4444;
    break;
  case D3DFMT_R5G6B5:         flags = sTEX_RGB565;
    break;
...
sInt sRenderStateTexture(sU32* data, sInt texstage, sU32 tflags, sF32 lodbias)
{
  ...
  {
  case sMTF_LEVEL0:
    *data++ = tex + D3DSAMP_MAGFILTER;
    *data++ = D3DTEXF_POINT;
    *data++ = tex + D3DSAMP_MINFILTER;
    *data++ = D3DTEXF_POINT;
    *data++ = tex + D3DSAMP_MIPFILTER;
    *data++ = D3DTEXF_POINT;
    break;
  case sMTF_LEVEL1:
    *data++ = tex + D3DSAMP_MAGFILTER;
  ...
  case sMTF_LEVEL3:
    ...
    if(DXCaps.TextureFilterCaps & D3DPTFILTERCAPS_MAGFANISOTROPIC)
    {
      ...
      *data++ = tex + D3DSAMP_MAXANISOTROPY;
      *data++ = DXCaps.MaxAnisotropy;
    }
    else
    {
      *data++ = tex + D3DSAMP_MAGFILTER;
  ...
  *data++ = tex + D3DSAMP_MIPMAPLODBIAS;
  *(sF32*)data = lodbias;
  data++;
  ...
  *data++ = tex + D3DSAMP_ADDRESSU;
  ...
  {
  case sMTF_TILE_U:   *data++ = D3DTADDRESS_WRAP;
    break;
  case sMTF_CLAMP_U:  *data++ = D3DTADDRESS_CLAMP;
    break;
  case sMTF_MIRROR_U: *data++ = D3DTADDRESS_MIRROR;
    break;
  case sMTF_BORDER_U:
  ...
  *data++ = tex + D3DSAMP_ADDRESSV;
  ...
  {
  case sMTF_TILE_V:   *data++ = D3DTADDRESS_WRAP;
    break;
  case sMTF_CLAMP_V:  *data++ = D3DTADDRESS_CLAMP;
    break;
  case sMTF_MIRROR_V: *data++ = D3DTADDRESS_MIRROR;
    break;
  case sMTF_BORDER_V:
    *data++ = D3DTADDRESS_BORDER;
    *data++ = tex + D3DSAMP_BORDERCOLOR;
  ...
  *data++ = tex + D3DSAMP_ADDRESSW;
  ...
  {
  case sMTF_TILE_W:   *data++ = D3DTADDRESS_WRAP;
    break;
  case sMTF_CLAMP_W:  *data++ = D3DTADDRESS_CLAMP;
    break;
  case sMTF_MIRROR_W: *data++ = D3DTADDRESS_MIRROR;
    break;
  case sMTF_BORDER_W:
    *data++ = D3DTADDRESS_BORDER;
    *data++ = tex + D3DSAMP_BORDERCOLOR;
...
void sVertexFormatHandle::Create()
{
  ...
  while(Data[i])
  {
    ...
    {
    case sVF_NOP:   break;
    case sVF_POSITION:    decl[j].Usage = D3DDECLUSAGE_POSITION;
      decl[j].UsageIndex = 0;
      j++;
      break;
    case sVF_NORMAL:      decl[j].Usage = D3DDECLUSAGE_NORMAL;
      decl[j].UsageIndex = 0;
      j++;
      break;
    case sVF_TANGENT:     decl[j].Usage = D3DDECLUSAGE_TANGENT;
      decl[j].UsageIndex = 0;
      j++;
      break;
    case sVF_BONEINDEX:   decl[j].Usage = D3DDECLUSAGE_BLENDINDICES;
      decl[j].UsageIndex = 0;
      j++;
      break;
    case sVF_BONEWEIGHT:  decl[j].Usage = D3DDECLUSAGE_BLENDWEIGHT;
      decl[j].UsageIndex = 0;
      j++;
      break;
    case sVF_BINORMAL:    decl[j].Usage = D3DDECLUSAGE_BINORMAL;
      decl[j].UsageIndex = 0;
      j++;
      break;
    case sVF_COLOR0:      decl[j].Usage = D3DDECLUSAGE_COLOR;
      decl[j].UsageIndex = 0;
      j++;
      break;
    case sVF_COLOR1:      decl[j].Usage = D3DDECLUSAGE_COLOR;
      decl[j].UsageIndex = 1;
      j++;
      break;
    case sVF_COLOR2:      decl[j].Usage = D3DDECLUSAGE_COLOR;
      decl[j].UsageIndex = 2;
      j++;
      break;
    case sVF_COLOR3:      decl[j].Usage = D3DDECLUSAGE_COLOR;
      decl[j].UsageIndex = 3;
      j++;
      break;
    case sVF_UV0:         decl[j].Usage = D3DDECLUSAGE_TEXCOORD;
      decl[j].UsageIndex = 0;
      j++;
      break;
    case sVF_UV1:         decl[j].Usage = D3DDECLUSAGE_TEXCOORD;
      decl[j].UsageIndex = 1;
      j++;
      break;
    case sVF_UV2:         decl[j].Usage = D3DDECLUSAGE_TEXCOORD;
      decl[j].UsageIndex = 2;
      j++;
      break;
    case sVF_UV3:         decl[j].Usage = D3DDECLUSAGE_TEXCOORD;
      decl[j].UsageIndex = 3;
      j++;
      break;
    case sVF_UV4:         decl[j].Usage = D3DDECLUSAGE_TEXCOORD;
      decl[j].UsageIndex = 4;
      j++;
      break;
    case sVF_UV5:         decl[j].Usage = D3DDECLUSAGE_TEXCOORD;
      decl[j].UsageIndex = 5;
      j++;
      break;
    case sVF_UV6:         decl[j].Usage = D3DDECLUSAGE_TEXCOORD;
      decl[j].UsageIndex = 6;
      j++;
      break;
    case sVF_UV7:         decl[j].Usage = D3DDECLUSAGE_TEXCOORD;
      decl[j].UsageIndex = 7;
      j++;
      break;
    default: sVERIFYFALSE;
    ...
    {
    case sVF_F2:  decl[i].Type = D3DDECLTYPE_FLOAT2;
      b[stream] += 2 * 4;
      break;
    case sVF_F3:  decl[i].Type = D3DDECLTYPE_FLOAT3;
      b[stream] += 3 * 4;
      break;
    case sVF_F4:  decl[i].Type = D3DDECLTYPE_FLOAT4;
      b[stream] += 4 * 4;
      break;
    case sVF_I4:  decl[i].Type = D3DDECLTYPE_UBYTE4;
      b[stream] += 1 * 4;
      break;
    case sVF_C4:  decl[i].Type = D3DDECLTYPE_D3DCOLOR;
      b[stream] += 1 * 4;
      break;
    case sVF_S2:  decl[i].Type = D3DDECLTYPE_SHORT2N;
      b[stream] += 1 * 4;
      break;
    case sVF_S4:  decl[i].Type = D3DDECLTYPE_SHORT4N;
      b[stream] += 2 * 4;
      break;
    case sVF_H2:  decl[i].Type = D3DDECLTYPE_FLOAT16_2;
      b[stream] += 1 * 4;
      break;
    case sVF_H4:  decl[i].Type = D3DDECLTYPE_FLOAT16_4;
      b[stream] += 2 * 4;
      break;
    case sVF_F1:  decl[i].Type = D3DDECLTYPE_FLOAT1;
      b[stream] += 1 * 4;
      break;
    default: sVERIFYFALSE;
    }
...
void sGeometry::InitDyn(sInt ic, sInt vc0, sInt vc1, sInt vc2, sInt vc3)
{
  if(ic)
  {
    IndexPart.Init(ic, IndexSize, sGD_DYNAMIC, IndexSize == 2 ? 1 : 2, 0);
    IndexPart.Buffer->Unlock();
  }

  if(vc0)
  {
    VertexPart[0].Init(vc0, Format->GetSize(0), sGD_DYNAMIC, 0, 0);
    VertexPart[0].Buffer->Unlock();
  }

  if(vc1)
  {
    VertexPart[1].Init(vc1, Format->GetSize(1), sGD_DYNAMIC, 0, 0);
    VertexPart[0].Buffer->Unlock();
  }

  if(vc2)
  {
    VertexPart[2].Init(vc2, Format->GetSize(2), sGD_DYNAMIC, 0, 0);
    VertexPart[0].Buffer->Unlock();
  }

  if(vc3)
  {
    VertexPart[3].Init(vc3, Format->GetSize(3), sGD_DYNAMIC, 0, 0);
    VertexPart[0].Buffer->Unlock();
  }
}
...
{
  if(discard)
    VertexPart[stream].Buffer->Discard = 1;
...
void sGeometry::Draw(const sGeometryDrawInfo& di)
{
  ...
  if(di.Flags & sGDI_BlendFactor)
  {
    sU32 data[2] =
    {
      sMS_RENDERSTATE + sInt(D3DRS_BLENDFACTOR), di.BlendFactor
    };
