...
{
  sExternCompileBuffer(sInt size0, sInt size1)
  {
    Buffer = new sU8[size0];
    BufferSize = size0;
    ResultSize = 0;
    Message = new sChar8[size1];
    Message[0] = 0;
    MessageSize = size1;
  }

  ~sExternCompileBuffer()
  {
    sDeleteArray(Buffer);
    sDeleteArray(Message);
  }
  ...
  void SetShader(const sU8* data, sInt size)
  {
    for(sInt i = 0; i < sMin(size, BufferSize); i++)
      Buffer[i] = data[i];

    ResultSize = size;
  }

  void SetMessage(const sChar8* msg)
  {
    sInt i = 0;

    for(; i < MessageSize - 1 && *msg; i++)
      Message[i] = *msg++;
