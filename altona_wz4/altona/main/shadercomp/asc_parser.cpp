...
sBool ACDoc::_IsDecl()
{
  if(Scan.Token == sTOK_NAME)
  {
    if(FindType(Scan.Name))
      return 1;
  }

  if(Scan.Token == AC_TOK_IN)
    return 1;

  if(Scan.Token == AC_TOK_OUT)
    return 1;

  if(Scan.Token == AC_TOK_INOUT)
    return 1;

  if(Scan.Token == AC_TOK_UNIFORM)
    return 1;

  if(Scan.Token == AC_TOK_ROWMAJOR)
    return 1;

  if(Scan.Token == AC_TOK_COLMAJOR)
    return 1;

  if(Scan.Token == AC_TOK_CONST)
    return 1;

  if(Scan.Token == AC_TOK_INLINE)
    return 1;

  if(Scan.Token == AC_TOK_STATIC)
    return 1;

  if(Scan.Token == AC_TOK_GROUPSHARED)
    return 1;
...
// usages and flags
  ...
  if(Scan.Token == AC_TOK_GROUPSHARED)
  {
    usages |= ACF_GROUPSHARED;
    Scan.Scan();
    goto usages_;
  }

  if(Scan.IfName(L"point"))
  {
    usages |= ACF_POINT;
    goto usages_;
  }

  if(Scan.IfName(L"line"))
  {
    usages |= ACF_LINE;
    goto usages_;
  }

  if(Scan.IfName(L"triangle"))
  {
    usages |= ACF_TRIANGLE;
    goto usages_;
  }

  if(Scan.IfName(L"lineadj"))
  {
    usages |= ACF_LINEADJ;
    goto usages_;
  }

  if(Scan.IfName(L"triangleadj"))
  {
    usages |= ACF_TRIANGLEADJ;
    goto usages_;
  }
...
sInt BinaryOp(sInt token, sInt& pri)
{
  ...
  case '-':               pri = 2;
    return ACE_SUB;
  case sTOK_SHIFTL:       pri = 3;
    return ACE_SHIFTL;
  case sTOK_SHIFTR:       pri = 3;
    return ACE_SHIFTR;
  case '>':               pri = 4;
    return ACE_GT;
  case '<':               pri = 4;
  ...
  case '|':               pri = 8;
    return ACE_BOR;
  case AC_TOK_IMPLIES:    pri = 9;
    return ACE_IMPLIES;
  case AC_TOK_ANDAND:     pri = 10;
    return ACE_AND;
  case AC_TOK_OROR:       pri = 11;
    return ACE_OR;
  case '?':               pri = 12;
    return ACE_COND1;
    /*
...
ACExpression* ACDoc::_Expression()
{
  ...
  case '=':               op = ACE_ASSIGN;
    break;
  case AC_TOK_ASSIGNADD:  op = ACE_ASSIGN_ADD;
    break;
  case AC_TOK_ASSIGNSUB:  op = ACE_ASSIGN_SUB;
    break;
  case AC_TOK_ASSIGNMUL:  op = ACE_ASSIGN_MUL;
    break;
  case AC_TOK_ASSIGNDIV:  op = ACE_ASSIGN_DIV;
    break;
  }
