...
class SpellChecker
{
  ...
  sBool isletter(sChar x)
  {
    return x >= 0 && x < 0x100 && IsLetter[x];
  }
...
public:
  ...
  void AddText(const MarkupStyle& s, sLBParagraph* p, const sChar* start)
  {
    AddText(s, p, start, start + sGetStringLen(start));
  }

  void AddText(const MarkupStyle& s, sLBParagraph* p, const MarkupText& text)
  {
    AddText(s, p, text.Start, text.Start + text.Count);
  }

  void AddText(sLBParagraph* p, const sChar* start, const sChar* end)
  {
    AddText(GlobalStyle, p, start, end);
  }

  void AddText(sLBParagraph* p, const sChar* start)
  {
    AddText(GlobalStyle, p, start, start + sGetStringLen(start));
  }

  void AddText(sLBParagraph* p, const MarkupText& text)
  {
    AddText(GlobalStyle, p, text.Start, text.Start + text.Count);
  }
...
public:
  ...
  MarkupTypePlain(const sChar* code, sF32 ad)
  {
    Code = code;
    Advance = ad;
  }
...
{
public:
  MarkupTypeExecute(const sChar* code)
  {
    Execute = 1;
    Verbatim = 1;
    Code = code;
...
public:
  MarkupWordStyle(const sChar* code, sInt s, sInt c)
  {
    Code = code;
    Set = s;
    Clr = c;
...
public:
  MarkupWordSize(const sChar* code, sInt i)
  {
    Code = code;
    Inc = i;
  }
  ...
  void ChangeStyle(MarkupStyle& s, const sChar*& t, sLBParagraph*, Markup*)
  {
    s.FontSize = sClamp(s.FontSize + Inc, 0, sRFS_MAX - 1);
    s.UpdateFont();
  }
};
...
{
public:
  MarkupWordCode(const sChar* code)
  {
    Code = code;
  }
...
{
public:
  MarkupWordTextColor(const sChar* code)
  {
    Code = code;
  }
...
{
public:
  MarkupWordBackColor(const sChar* code)
  {
    Code = code;
  }
...
{
public:
  MarkupWordHeadColor(const sChar* code)
  {
    Code = code;
  }
...
{
public:
  MarkupWordLineColor(const sChar* code)
  {
    Code = code;
  }
...
public:
  MarkupWordFontFamily(const sChar* code, sInt fam)
  {
    Code = code;
    Family = fam;
  }
  ...
  void ChangeStyle(MarkupStyle& s, const sChar*& t, sLBParagraph*, Markup*)
  {
    ...
    if(Family == sRFF_SYMBOL)
      s.FontStyle |= sF2C_SYMBOLS;
...
{
public:
  MarkupWordImage(const sChar* code)
  {
    Code = code;
  }
...
public:
  MarkupWordLink(const sChar* code, const sChar* cmd, sU32 tc)
  {
    Code = code;
    Command = cmd;
    TextColor = tc;
...
public:
  MarkupWordAlign(const sChar* code, sInt align)
  {
    Code = code;
    Align = align;
  }
...
{
public:
  MarkupWordDict(const sChar* code)
  {
    Code = code;
  }
