...
SpellChecker::SpellChecker()
{
  ...
  for(sInt i = 0xa0; i <= 0xff; i++)
    IsLetter[i] = 1;

  sClear(LowerLetter);

  for(sInt i = 'A'; i <= 'Z'; i++)
    LowerLetter[i] = i + 0x20;

  for(sInt i = 0xc0; i <= 0xdf; i++)
    LowerLetter[i] = i + 0x20;
...
void MarkupStyle::SetBox(sLayoutBox* box) const
{
  if(!LeftAction.IsEmpty())
    box->LeftAction = LeftAction;

  if(!RightAction.IsEmpty())
    box->RightAction = RightAction;

  if(!MiddleAction.IsEmpty())
    box->MiddleAction = MiddleAction;
}
...
void Markup::AddTextR(const MarkupStyle& style, const MarkupStyle* oldstyle, sLBParagraph* par, const sChar* s, const sChar* e)
{
  do
  {
    ...
    while(ps < s)
    {
      ...
      while(n0 < s && sIsSpace(*n0))
        n0 += (n0[0] == '\\' && n0[1] >= 32) ? 2 : 1;

      const sChar* n1 = n0;

      while(n1 < s && !sIsSpace(*n1))
        n1 += (n1[0] == '\\' && n1[1] >= 32) ? 2 : 1;
      ...
      if(n0 < n1)
      {
        ...
        if(style.Dict)
          word->WrongSpelling = !Check(style.Dict, word->Text, word->Length);
...
sLayoutBox* MarkupTypeItems::BeginBoxes(MarkupBlock* block, sLayoutBox* box, Markup* markup)
{
  ...
  while(str < end)
  {
    while(sIsSpace(*str) && str < end)
      str++;

    sInt n = 0;

    while(str + n < end && str[n] != '|')
      n++;

    sInt m = n;

    while(m > 0 && sIsSpace(str[m - 1]))
      m--;
...
{
  sLBSpacer* space = new sLBSpacer(1, markup->GlobalStyle.Font->Font->GetHeight(), markup->GlobalStyle.BackColor);
  box->AddChild(space);
...
sLayoutBox* MarkupTypeSvnConflict::BeginBoxes(MarkupBlock* block, sLayoutBox* box, Markup* markup)
{
  ...
  case 1:
    ...
    space = new sLBSpacer(1, markup->GlobalStyle.Font->Font->GetHeight(), markup->GlobalStyle.BackColor);
    box->AddChild(space);
    break;
  case 2:
    space = new sLBSpacer(1, markup->GlobalStyle.Font->Font->GetHeight(), markup->GlobalStyle.BackColor);
...
sLayoutBox* MarkupTypeCode::BeginBoxes(MarkupBlock* block, sLayoutBox* box, Markup* markup)
{
  ...
  if(block->Text.Start)
  {
    ...
    while(t < e)
    {
      ...
      if(style.Dict)
        word->WrongSpelling = !markup->Check(style.Dict, word->Text, word->Length);
...
{
  if(d->Check(word, length))
    return 1;

  if(PageDict.Check(word, length))
    return 1;
...
sLayoutBox* MarkupTypeChapterFormfeed::BeginBoxes(MarkupBlock* block, sLayoutBox* box, Markup* markup)
{
  ...
  if(str < end && *str == '0')
    markup->GlobalStyle.ChapterFormfeed = 0;
  else
    markup->GlobalStyle.ChapterFormfeed = 1;
...
void MarkupWordLink::ChangeStyle(MarkupStyle& s, const sChar*& t, sLBParagraph* par, Markup* markup)
{
  ...
  if(par)
  {
    if(!GetString(t, name))
    {
      ...
      while(sIsDigit(*text) || sIsLetter(*text))
        text++;
