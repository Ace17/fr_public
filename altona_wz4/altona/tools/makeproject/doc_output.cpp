...
void Document::OutputFilterFiles(ProjFolder& folder, const sChar* path, sInt group)
{
  ...
  if(group > 0)
    gname = Targets[group - 1]->Name;

  File* file;
  sFORALL(folder.Files, file)
...
void Document::OutputDependencies(ProjectData* project, sArray<ProjectData*>& done)
{
  ...
  sFORALL(project->Dependencies, dep)
  {
    ...
    for(sInt i = 0; i < done.GetCount(); i++)
      if(done[i] == dep->Project)
        found = sTRUE;
...
void Document::OutputXProject() // for VS2010 and higher
{
  ...
  static const sU32 platflags[] =
  {
    sVSPLAT_WIN32, sVSPLAT_WIN64
  };
  static const sChar* platnames[] =
  {
    L"Win32", L"x64",
  };
  ...
  FORALLCONFIG(conf)
  {
    static const sChar* conftypes[] =
    {
      L"", L"Application", L"DynamicLibrary", L"", L"StaticLibrary"
    };
    ...
    if(VS_Version == 11)
      OutBuffer.PrintF(L"\t\t<PlatformToolset>v110</PlatformToolset>\r\n");
  ...
  for(sInt i = 0; i <= Targets.GetCount(); i++)
    groups[i] = 0;
  ...
  for(sInt g = Targets.GetCount(); g >= 0; g--)
    if(groups[g])
    {
      sPoolString name = L"None";

      if(g > 0)
        name = Targets[g - 1]->Name;
