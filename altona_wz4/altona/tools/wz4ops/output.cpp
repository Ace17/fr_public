...
void Document::OutputExpr(ExprNode* node)
{
  ...
  switch(node->Op)
  {
  case EOP_LE:     OutputExpr(node->Left);
    CPP.Print(L"<=");
    OutputExpr(node->Right);
    break;
  case EOP_GE:     OutputExpr(node->Left);
    CPP.Print(L">=");
    OutputExpr(node->Right);
    break;
  case EOP_LT:     OutputExpr(node->Left);
    CPP.Print(L"<");
    OutputExpr(node->Right);
    break;
  case EOP_GT:     OutputExpr(node->Left);
    CPP.Print(L">");
    OutputExpr(node->Right);
    break;

  case EOP_EQ:     OutputExpr(node->Left);
    CPP.Print(L"==");
    OutputExpr(node->Right);
    break;
  case EOP_NE:     OutputExpr(node->Left);
    CPP.Print(L"!=");
    OutputExpr(node->Right);
    break;
  case EOP_AND:    OutputExpr(node->Left);
    CPP.Print(L"&&");
    OutputExpr(node->Right);
    break;
  case EOP_OR:     OutputExpr(node->Left);
    CPP.Print(L"||");
    OutputExpr(node->Right);
    break;
  case EOP_BITAND: OutputExpr(node->Left);
    CPP.Print(L"&");
    OutputExpr(node->Right);
    break;
  case EOP_BITOR:  OutputExpr(node->Left);
    CPP.Print(L"|");
    OutputExpr(node->Right);
    break;

  case EOP_NOT:    CPP.Print(L"!");
    OutputExpr(node->Left);
    break;
  case EOP_BITNOT: CPP.Print(L"~");
    OutputExpr(node->Left);
    break;

  case EOP_INT:    CPP.PrintF(L"0x%04x", node->Value);
    break;
  case EOP_SYMBOL: CPP.PrintF(L"para->%s", node->Symbol);
    break;
  case EOP_INPUT:  CPP.PrintF(L"(op->ConnectionMask & (1<<%d))!=0", node->Value);
    break;
  }
...
// CPP.Print(L"  gh.PushButton(L\"select all\",gh.ArraySelectAllMsg);\n");
    ...
    if(op->Parameters.GetCount() > 0)
    {
      sFORALL(op->Parameters, para)
      {
        ...
        {
        case TYPE_BITMASK:
        case TYPE_INT:      tname = L"int";
          break;
        case TYPE_FLOAT:    tname = L"float";
          break;
        case TYPE_COLOR:    tname = L"color";
          break;
        case TYPE_STRING:
        case TYPE_FILEIN:
        case TYPE_FILEOUT:  tname = L"string";
          break;
        }
...
// CPP.PrintF(L"  %sType->Init();\n",type->Symbol);  // now done in constructor
  ...
  sFORALL(Ops, op)
  {
    ...
    sFORALL(op->Inputs, in)
    {
      ...
      switch(in->Method)
      {
      case IE_LINK: CPP.PrintF(L"|wCIF_METHODLINK");
        break;
      case IE_BOTH: CPP.PrintF(L"|wCIF_METHODBOTH");
        break;
      case IE_CHOOSE: CPP.PrintF(L"|wCIF_METHODCHOOSE");
        break;
      case IE_ANIM: CPP.PrintF(L"|wCIF_METHODANIM");
        break;
      }
