 ...
 */
  ...
  {
    sTextBuffer tb;
    static const sChar* mode[2] =
    {
      L"off", L"on "
    };
    static const sChar* zmode[3] =
    {
      L"smooth", L"initial picture integral zoom", L"only integral zoom"
    };
 ...
 */
  ...
  case sKEY_SPACE:
    CmdReset();
    break;
  case sKEY_LMB: MouseB |= 1;
    break;
  case sKEY_RMB: MouseB |= 2;
    break;
  case sKEY_MMB: MouseB |= 4;
    break;
  case sKEY_LMB | sKEYQ_BREAK: MouseB &= ~1;
    break;
  case sKEY_RMB | sKEYQ_BREAK: MouseB &= ~2;
    break;
  case sKEY_MMB | sKEYQ_BREAK: MouseB &= ~4;
    break;
...
void MyApp::GoImage(sInt id)
{
  ...
  if(img->SizeX <= sx && img->SizeY <= sy)
    flags |= sTEX_NOMIPMAPS;
