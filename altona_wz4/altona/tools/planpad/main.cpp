    ...
    if(box)
    {
      ...
      if(dd.Mode == sDD_START)
      {
        ...
        case 1:
          ...
          if(!box->LeftAction.IsEmpty())
            FakeScript(box->LeftAction, 0);

          break;
        case 2:

          if(!box->RightAction.IsEmpty())
            FakeScript(box->RightAction, 0);

          break;
        case 4:

          if(!box->MiddleAction.IsEmpty())
            FakeScript(box->MiddleAction, 0);
