...
static const sChar* shadername[] =
{
  L"VS", L"HS", L"DS", L"GS", L"PS", L"CS"
};
static const sChar* fullshadername[] =
{
  L"VertexShader", L"HullShader", L"DomainShader", L"GeometryShader", L"PixelShader", L"ComputeShader"
};
...
void sPrintNewShaderCode(sTextBuffer& CPP, const sChar* name, const sChar* size, NewShader* ns)
{
  ...
  {
  case sSTF_VERTEX:   st = L"sSTF_VERTEX";
    break;
  case sSTF_PIXEL:    st = L"sSTF_PIXEL";
    break;
  case sSTF_GEOMETRY: st = L"sSTF_GEOMETRY";
    break;
  case sSTF_HULL:     st = L"sSTF_HULL";
    break;
  case sSTF_DOMAIN:   st = L"sSTF_DOMAIN";
    break;
  case sSTF_COMPUTE:  st = L"sSTF_COMPUTE";
    break;
  }
    ...
    if(type->Type == ACT_CBUFFER)
    {
      ...
      sFORALL(type->Members, member)
      {
        ...
        if(c == 4 && r == 4)
          ...
          name = L"sMatrix34CM"; // column major stored sMatrix34
        else
          ac *= c;
      ...
      static const sChar* shaderconst[] =
      {
        L"sCBUFFER_VS", L"sCBUFFER_PS", L"sCBUFFER_GS", L"sCBUFFER_HS", L"sCBUFFER_DS", L"sCBUFFER_CS"
      };
