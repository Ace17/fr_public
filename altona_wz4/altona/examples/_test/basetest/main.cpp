...
void TestStringFunctions()
{
  ...
  sCopyString(b0, L"abcd", 256);
  sAppendString(b0, L"1234", 10);
  CHECK(sCmpString(b0, L"abcd1234") == 0);
  sCopyString(b0, L"abcd", 256);
  sAppendString(b0, L"1234", 9);
  CHECK(sCmpString(b0, L"abcd1234") == 0);
  sCopyString(b0, L"abcd", 256);
  sAppendString(b0, L"1234", 8);
  CHECK(sCmpString(b0, L"abcd123") == 0);
  sCopyString(b0, L"abcd", 256);
  sAppendString(b0, L"1234", 7);
...
void TestString()
{
  ...
  t1.CutLeft(4);
  CHECK(sCmpString(t1, L"quick brown fox jumps over the lazy dog") == 0);
  t1.CutRight(4);
  CHECK(sCmpString(t1, L"quick brown fox jumps over the lazy") == 0);
  t1.CutRange(6, 6);
  CHECK(sCmpString(t1, L"quick fox jumps over the lazy") == 0);
  t1.CutRange(0, 5);
  CHECK(sCmpString(t1, L" fox jumps over the lazy") == 0);
  t1.StripLeft();
  CHECK(sCmpString(t1, L"fox jumps over the lazy") == 0);
  t1.CutRange(19, 4 + 666);
  CHECK(sCmpString(t1, L"fox jumps over the ") == 0);
  t1.StripRight();
  CHECK(sCmpString(t1, L"fox jumps over the") == 0);
  t1.StripLeft(L"fox");
  CHECK(sCmpString(t1, L" jumps over the") == 0);
  t1.StripRight(L"the");
  ...
  CHECK(sCmpString(t1, L"jumps over") == 0);
  t1.CutRange(-2, 4);
  CHECK(sCmpString(t1, L"mps over") == 0);
  t1.CutRange(-2, 2);
  CHECK(sCmpString(t1, L"mps over") == 0);
  t1.CutRange(10, 2);
  CHECK(sCmpString(t1, L"mps over") == 0);
  ...
  CHECK(sCmpString(t1, L"mps over") == 0);
  t1.CutRange(2, -3);
  CHECK(sCmpString(t1, L"s over") == 0);
  t1.CutRange(4, -2);
  CHECK(sCmpString(t1, L"s er") == 0);
  t1.CutRange(66, -200);
  CHECK(sCmpString(t1, L"") == 0);
  t1.PadRight(3, L'a');
  CHECK(sCmpString(t1, L"aaa") == 0);
  t1.PadRight(4);
  CHECK(sCmpString(t1, L"aaa ") == 0);
  t1.PadRight(2);
  CHECK(sCmpString(t1, L"aaa ") == 0);
...
void TestFormat()
{
  ...
  sSPrintF(str, L"|%x|", 0xabc);
  CHECK(str == L"|abc|");
  sSPrintF(str, L"|%x|", -0xabc);
  CHECK(str == L"|-abc|");
  sSPrintF(str, L"|%x|", sU32(-0xabc));
  CHECK(str == L"|fffff544|");
  sSPrintF(str, L"|%d|", 123);
  CHECK(str == L"|123|");
  sSPrintF(str, L"|%d|", -123);
  CHECK(str == L"|-123|");
  sSPrintF(str, L"|%_|", 3);
  ...
  sSPrintF(str, L"|%x|", sU64(0x0123456789abcdefULL));
  CHECK(str == L"|123456789abcdef|");
  sSPrintF(str, L"|%x|", sS64(0x0123456789abcdefULL));
  CHECK(str == L"|123456789abcdef|");
  sSPrintF(str, L"|%x|", sS64(-0x0123456789abcdefLL));
  CHECK(str == L"|-123456789abcdef|");

  sSPrintF(str, L"|%d|", sU64(1234567812345678ULL));
  CHECK(str == L"|1234567812345678|");
  sSPrintF(str, L"|%d|", sS64(1234567812345678ULL));
  CHECK(str == L"|1234567812345678|");
  sSPrintF(str, L"|%d|", sS64(-1234567812345678LL));
  CHECK(str == L"|-1234567812345678|");

  sSPrintF(str, L"|%6d|", 123);
  CHECK(str == L"|   123|");
  sSPrintF(str, L"|%6d|", -123);
  CHECK(str == L"|  -123|");
  sSPrintF(str, L"|%06d|", 123);
  CHECK(str == L"|000123|");
  sSPrintF(str, L"|%06d|", -123);
  CHECK(str == L"|-00123|");
  sSPrintF(str, L"|%-6d|", 123);
  ...
  sSPrintF(str, L"|%5.2r|", 1);
  CHECK(str == L"| 0.01|");
  sSPrintF(str, L"|%5.2r|", 12);
  CHECK(str == L"| 0.12|");
  sSPrintF(str, L"|%5.2r|", 123);
  CHECK(str == L"| 1.23|");
  sSPrintF(str, L"|%5.2r|", 1234);
  CHECK(str == L"|12.34|");
  sSPrintF(str, L"|%05.2r|", 1);
  CHECK(str == L"|00.01|");
  sSPrintF(str, L"|%05.2r|", 12);
  CHECK(str == L"|00.12|");
  sSPrintF(str, L"|%05.2r|", 123);
  CHECK(str == L"|01.23|");
  sSPrintF(str, L"|%05.2r|", 1234);
  CHECK(str == L"|12.34|");
  sSPrintF(str, L"|%.2r|", 1);
  CHECK(str == L"|0.01|");
  sSPrintF(str, L"|%.2r|", 12);
  CHECK(str == L"|0.12|");
  sSPrintF(str, L"|%.2r|", 123);
  CHECK(str == L"|1.23|");
  sSPrintF(str, L"|%.2r|", 1234);
  CHECK(str == L"|12.34|");

  sSPrintF(str, L"|%8.3f|", 12.3f);
  CHECK(str == L"|  12.300|");
  sSPrintF(str, L"|%8.3f|", -12.3f);
  CHECK(str == L"| -12.300|");
  sSPrintF(str, L"|%8.3f|", 0.123f);
  CHECK(str == L"|   0.123|");
  sSPrintF(str, L"|%8.3f|", 123.0f);
  CHECK(str == L"| 123.000|");
...
// sSPrintF(str,L"|%24.8f|",12.3f);                 CHECK(str==L"|         12.30000000|");
  sSPrintF(str, L"|%8.2f|%8.4f|", 1.004f, 1.004f);
  CHECK(str == L"|    1.00|  1.0040|");
  sSPrintF(str, L"|%8.2f|%8.4f|", 1.005f, 1.005f);
  CHECK(str == L"|    1.01|  1.0050|");
  sSPrintF(str, L"|%8.2f|%8.4f|", 1.006f, 1.006f);
  CHECK(str == L"|    1.01|  1.0060|");
  sSPrintF(str, L"|%8.2f|%8.4f|", 1.009f, 1.009f);
  CHECK(str == L"|    1.01|  1.0090|");
  sSPrintF(str, L"|%8.2f|%8.4f|", 1.010f, 1.010f);
  CHECK(str == L"|    1.01|  1.0100|");
  sSPrintF(str, L"|%8.2f|%8.4f|", 1.011f, 1.011f);
  CHECK(str == L"|    1.01|  1.0110|");
  sSPrintF(str, L"|%8f|", 12.345f);
  CHECK(str == L"|12.34500|");
  sSPrintF(str, L"|%6f|", 12.345f);
  CHECK(str == L"|12.345|");
  sSPrintF(str, L"|%5f|", 12.345f);
  CHECK(str == L"|12.35|");
  sSPrintF(str, L"|%8f|", -12.34f);
  CHECK(str == L"|-12.3400|");
  sSPrintF(str, L"|%6f|", -12.34f);
  CHECK(str == L"|-12.34|");
  sSPrintF(str, L"|%5f|", -12.34f);
  CHECK(str == L"|-12.3|");

  sSPrintF(str, L"|%8.3f|", 64.0f);
  CHECK(str == L"|  64.000|");
  sSPrintF(str, L"|%8.3f|", 99.0f - 0.0001f);
  CHECK(str == L"|  99.000|");
  sSPrintF(str, L"|%8.3f|", 100.0f - 0.0001f);
  CHECK(str == L"| 100.000|");

  sSPrintF(str, L"|%5f|", 100000.0f);
  CHECK(str == L"|100000|");
  sSPrintF(str, L"|%5f|", 10000.0f);
  CHECK(str == L"|10000|");
  sSPrintF(str, L"|%5f|", 1000.0f);
  CHECK(str == L"| 1000|");
  sSPrintF(str, L"|%5f|", 100.0f);
  CHECK(str == L"|100.0|");
  sSPrintF(str, L"|%5f|", 10.0f);
  CHECK(str == L"|10.00|");
  sSPrintF(str, L"|%5f|", 1.0f);
  CHECK(str == L"|1.000|");
  sSPrintF(str, L"|%5f|", 0.1f);
  CHECK(str == L"|0.100|");
  sSPrintF(str, L"|%5f|", 0.01f);
  CHECK(str == L"|0.010|");
  sSPrintF(str, L"|%5f|", 0.001f);
  CHECK(str == L"|0.001|");
  sSPrintF(str, L"|%5f|", 0.0001f);
  CHECK(str == L"|0.000|");
  sSPrintF(str, L"|%5f|", 0.00001f);
  CHECK(str == L"|0.000|");
  sSPrintF(str, L"|%5f|", 0.000001f);
  CHECK(str == L"|0.000|");

  sSPrintF(str, L"|%5.2f|", 100000.0f);
  CHECK(str == L"|100000.00|");
  sSPrintF(str, L"|%5.2f|", 10000.0f);
  CHECK(str == L"|10000.00|");
  sSPrintF(str, L"|%5.2f|", 1000.0f);
  CHECK(str == L"|1000.00|");
  sSPrintF(str, L"|%5.2f|", 100.0f);
  CHECK(str == L"|100.00|");
  sSPrintF(str, L"|%5.2f|", 10.0f);
  CHECK(str == L"|10.00|");
  sSPrintF(str, L"|%5.2f|", 1.0f);
  CHECK(str == L"| 1.00|");
  sSPrintF(str, L"|%5.2f|", 0.1f);
  CHECK(str == L"| 0.10|");
  sSPrintF(str, L"|%5.2f|", 0.01f);
  CHECK(str == L"| 0.01|");
  sSPrintF(str, L"|%5.2f|", 0.001f);
  CHECK(str == L"| 0.00|");

  sSPrintF(str, L"|%s|", L"abc");
  CHECK(str == L"|abc|");
  ...
  sVector30 v30;
  v30.Init(1.42f, 1.52f, 1.25f);
  sVector31 v31;
  v31.Init(1.42f, 1.52f, 1.25f);
  sVector4 v4;
  v4.Init(1.42f, 1.52f, 1.25f, 1.26f);

  sSPrintF(str, L"|%6.3f|", v30);
  CHECK(str == L"|[ 1.420, 1.520, 1.250,0]|");
  sSPrintF(str, L"|%6.3f|", v31);
  CHECK(str == L"|[ 1.420, 1.520, 1.250,1]|");
  sSPrintF(str, L"|%6.3f|", v4);
  CHECK(str == L"|[ 1.420, 1.520, 1.250, 1.260]|");
  ...
  five.PrintF(L"abcde");
  CHECK(five == L"abcde");
  five.PrintF(L"abcdef");
  CHECK(five == L"abcd?");
  five.PrintF(L"a%db", 10);
  CHECK(five == L"a10b");
  five.PrintF(L"a%db", 100);
  CHECK(five == L"a100b");
  five.PrintF(L"a%db", 1000);
  CHECK(five == L"a100?");
  five.PrintF(L"a%db%dc", 1, 2);
  CHECK(five == L"a1b2c");
  five.PrintF(L"a%db%dc", 10, 2);
  CHECK(five == L"a10b?");
  five.PrintF(L"a%db%dc", 10, 20);
  CHECK(five == L"a10b?");
}
...
typedef struct testStruct_
{
  sInt i;
  sString<10> s;
  testStruct_(sInt i_, sString<10> s_)
  {
    i = i_;
    s = s_;
  };
...
void TestStringMap()
{
  ...
  {
    sString<10> kbuf;
    sSPrintF(kbuf, L"key%03d", i);
    sString<10> vbuf;
    sSPrintF(vbuf, L"value%03d", i);
  ...
  sInt shuffle[20] =
  {
    13, 17, 4, 1, 12, 10, 8, 7, 3, 19, 2, 9, 11, 0, 14, 6, 15, 16, 5, 18
  };

  for(sInt i = 0; i < 20; i++)
  {
    sString<10> kbuf;
    sSPrintF(kbuf, L"key%03d", shuffle[i]);
    sString<10> vbuf;
    sSPrintF(vbuf, L"value%03d", shuffle[i]);
...
sU32 hookmask;
void hook0a(sInt a, void* b, void* user)
{
  hookmask |= 1;
}

void hook1a(sInt a, void* b, void* user)
{
  hookmask |= 2;
}

void hook2a(sInt a, void* b, void* user)
{
  hookmask |= 4;
}

void hook3a(sInt a, void* b, void* user)
{
  hookmask |= 8;
}
...
void testread(sReader& stream)
{
  ...
  stream.ArrayU32(buffer, 8);
  CHECK(sCmpMem(data, buffer, 8 * 4) == 0);
...
template<class Rand>
void TestRandom(const sChar* name)
{
  Rand rnd;
  sInt ok;
