...
sBool RequestHandler(sMiniFTPServer::RequestInfo& info)
{
  ...
  case sMFC_GET:    info.File = fnOk ? sCreateFile(filename, sFA_READ) : 0;
    return info.File != 0;
  case sMFC_PUT:    info.File = fnOk ? sCreateFile(filename, sFA_READWRITE) : 0;
    return info.File != 0;
  case sMFC_LIST:   sCopyMem(info.DirListing.AddMany(nListing), listing, nListing * 2);
    return sTRUE;
  }
